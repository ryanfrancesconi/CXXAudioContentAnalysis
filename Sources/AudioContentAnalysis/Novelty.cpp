...
class CNoveltyFromFile : public CNoveltyIf {
  public:
    CNoveltyFromFile(Novelty_t eNoveltyIdx, std::string strAudioFilePath,
...
CNoveltyFromFile::CNoveltyFromFile(Novelty_t eNoveltyIdx,
                                   std::string strAudioFilePath,
                                   int iBlockLength, int iHopLength)
    : m_pCAudioFile(0) {
    this->reset_();
...
/*! \brief class for computation of the novelty function from a vector of audio
 * data
 */
class CNoveltyFromVector : public CNoveltyIf {
  public:
    CNoveltyFromVector(Novelty_t eNoveltyIdx, const float *pfAudio,
                       long long iAudioLength, float fSampleRate,
                       int iBlockLength, int iHopLength);
    virtual ~CNoveltyFromVector() {};
};

CNoveltyFromVector::CNoveltyFromVector(Novelty_t eNoveltyIdx,
                                       const float *pfAudio,
...
/////////////////////////////////////////////////////////////////////////////////
// base class
CNoveltyIf::CNoveltyIf() { reset_(); }

inline CNoveltyIf::~CNoveltyIf() { reset_(); }

Error_t CNoveltyIf::create(CNoveltyIf *&pCInstance, Novelty_t eNoveltyIdx,
    ...
    pCInstance = new CNoveltyFromFile(eNoveltyIdx, strAudioFilePath,
...
Error_t CNoveltyIf::create(CNoveltyIf *&pCInstance, Novelty_t eNoveltyIdx,
    ...
    pCInstance = new CNoveltyFromVector(eNoveltyIdx, pfAudio, iNumSamples,
                                        fSampleRate, iBlockLength, iHopLength);
...
Error_t CNoveltyIf::compNovelty(float *pfNovelty, bool *pbIsOnset) {
    ...
    float *pfThreshold =
        0; //!< adaptive threshold - memory allocation is ok since we compDtw
           //!< the whole signal at once anyway
    ...
    m_pCLpFilter->reset();
    m_pCLpFilter->setFilterParam(m_pCLpFilter->calcFilterLength(
        fSmoothLpLenInS, m_fSampleRate / m_iHopLength));
    ...
    m_pCLpFilter->reset();
    m_pCLpFilter->setFilterParam(m_pCLpFilter->calcFilterLength(
        fThreshLpLenInS, m_fSampleRate / m_iHopLength));
    m_pCLpFilter->filtfilt(pfThreshold, pfNovelty, iNumBlocks);
    CVector::addC_I(pfThreshold,
                    .4F * CVector::getMean(&pfNovelty[1], iNumBlocks - 1),
                    iNumBlocks);
...
CNoveltyIf::Novelty_t
CNoveltyIf::getNoveltyIdxFromString(std::string sNoveltyString) {
...
void CNoveltyIf::computeMagSpectrum_() {
    ...
    CVector::mulC_I(m_pfProcBuff1, 2.F,
...
Error_t CNoveltyIf::init_(Novelty_t eNoveltyIdx) {
    ...
    CNoveltyFromBlockIf::create(m_pCNovelty, eNoveltyIdx,
                                m_pCFft->getLength(CFft::kLengthMagnitude),
                                m_fSampleRate);
