...
Error_t CSeqFeatureSel::process(const float *const *const ppfFeatures,
    ...
    for (auto f = iNumSelFeatures; f < iNumFeatures2Select; f++) {
        ...
        for (auto v = 0; v < m_iNumFeatures; v++) {
            ...
            CVector::copy(m_ppfTrain[iNumSelFeatures], ppfFeatures[v],
                          m_iNumObs);
        ...
        assert(m_piSelFeatures[iNumSelFeatures] >= 0 &&
               m_piSelFeatures[iNumSelFeatures] < m_iNumFeatures);
        CVector::copy(m_ppfTrain[iNumSelFeatures],
                      ppfFeatures[m_piSelFeatures[iNumSelFeatures]], m_iNumObs);
        iNumSelFeatures++;
    }
...
Error_t CSeqFeatureSel::getResult(int *piFeatureIndices,
                                  float *pfAccuracyPerStep /*= 0*/) {
    if (!m_bWasProcessed)
        return Error_t::kFunctionIllegalCallError;
    if (!piFeatureIndices)
        return Error_t::kFunctionInvalidArgsError;
