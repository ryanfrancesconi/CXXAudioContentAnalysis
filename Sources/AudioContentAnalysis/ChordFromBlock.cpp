...
Error_t CChordFromBlockIf::compChordProb(float *pfChordProb,
                                         const float *pfIn) {
    ...
    CMatrix::mulMatColvec(pfChordProb, m_ppfTemplateMatrix, m_pfPitchChroma,
                          CChordIf::kNumChords, kNumPitchClasses);

    assert(CVector::getSum(pfChordProb, CChordIf::kNumChords) > 0);

    // normalize to probability of 1
    CVector::mulC_I(pfChordProb,
                    1.F / CVector::getSum(pfChordProb, CChordIf::kNumChords),
                    CChordIf::kNumChords);

    return Error_t::kNoError;
}

inline CChordFromBlockIf::CChordFromBlockIf(int iMagSpecLength,
                                            float fSampleRate)
    ...
    CFeatureFromBlockIf::create(m_pCFeatureExtractor,
                                CFeatureIf::kFeatureSpectralPitchChroma,
                                m_iMagSpecLength, m_fSampleRate);
...
void CChordFromBlockIf::genTemplateMatrix_() {
    ...
    for (auto c = 0; c < kNumPitchClasses; c++) {
        for (auto p = 0; p < iNumChordPitches; p++) {
            m_ppfTemplateMatrix[c][(c + aiMajorIndices[p]) % 12] =
                1.F / iNumChordPitches;
            m_ppfTemplateMatrix[c + kNumPitchClasses]
                               [(c + aiMinorIndices[p]) % 12] =
                                   1.F / iNumChordPitches;
        }
    }
    CVector::addC_I(m_ppfTemplateMatrix[CChordIf::kNoChord],
                    1.F / kNumPitchClasses, kNumPitchClasses);
}
