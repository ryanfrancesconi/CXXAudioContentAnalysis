...
class CPitchFromFile : public CPitchIf {
  public:
    CPitchFromFile(PitchExtractors_t ePitchIdx, std::string strAudioFilePath,
...
CPitchFromFile::CPitchFromFile(PitchExtractors_t ePitchIdx,
                               std::string strAudioFilePath, int iBlockLength,
                               int iHopLength)
    : m_pCAudioFile(0) {
    this->reset_();
...
class CPitchFromVector : public CPitchIf {
  public:
    CPitchFromVector(PitchExtractors_t ePitchIdx, const float *pfAudio,
                     long long iAudioLength, float fSampleRate,
                     int iBlockLength, int iHopLength);
    virtual ~CPitchFromVector() {};
};

CPitchFromVector::CPitchFromVector(PitchExtractors_t ePitchIdx,
                                   const float *pfAudio, long long iAudioLength,
...
/////////////////////////////////////////////////////////////////////////////////
// base class
CPitchIf::CPitchIf() { reset_(); }

inline CPitchIf::~CPitchIf() { reset_(); }

Error_t CPitchIf::create(CPitchIf *&pCInstance, PitchExtractors_t ePitchIdx,
    ...
    pCInstance = new CPitchFromFile(ePitchIdx, strAudioFilePath, iBlockLength,
                                    iHopLength);
...
Error_t CPitchIf::create(CPitchIf *&pCInstance, PitchExtractors_t ePitchIdx,
    ...
    pCInstance = new CPitchFromVector(ePitchIdx, pfAudio, iNumSamples,
...
CPitchIf::PitchExtractors_t
CPitchIf::getPitchIdxFromString(std::string sPitchString) {
...
Error_t CPitchIf::init_(PitchExtractors_t ePitchIdx) {
    if (isPitchExtractorSpectral_(ePitchIdx)) {
        ...
        CPitchFromBlockIf::create(m_pCPitch, ePitchIdx,
                                  m_pCFft->getLength(CFft::kLengthMagnitude),
                                  m_fSampleRate);
    } else {
        // allocate processing memory
        CVector::alloc(m_pfProcBuff1, m_iBlockLength);
        CPitchFromBlockIf::create(m_pCPitch, ePitchIdx, m_iBlockLength,
                                  m_fSampleRate);
    }
