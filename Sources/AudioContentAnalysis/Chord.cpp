...
class CChordFromFile : public CChordIf {
  public:
    CChordFromFile(std::string strAudioFilePath, int iBlockLength,
                   int iHopLength);
...
CChordFromFile::CChordFromFile(std::string strAudioFilePath, int iBlockLength,
                               int iHopLength)
    : m_pCAudioFile(0) {
    this->reset_();
...
/*! \brief class for computation of chord progression from a vector of audio
 * data
 */
class CChordFromVector : public CChordIf {
  public:
    CChordFromVector(const float *pfAudio, long long iAudioLength,
                     float fSampleRate, int iBlockLength, int iHopLength);
    virtual ~CChordFromVector() {};
};

CChordFromVector::CChordFromVector(const float *pfAudio, long long iAudioLength,
...
CChordIf::CChordIf() { reset_(); }

inline CChordIf::~CChordIf() { reset_(); }

Error_t CChordIf::create(CChordIf *&pCInstance,
                         const std::string &strAudioFilePath, int iBlockLength,
                         int iHopLength) {
...
Error_t CChordIf::create(CChordIf *&pCInstance, const float *pfAudio,
                         long long iNumSamples, float fSampleRate,
    ...
    pCInstance = new CChordFromVector(pfAudio, iNumSamples, fSampleRate,
...
void CChordIf::computeMagSpectrum_() {
    ...
    CVector::mulC_I(m_pfProcBuff2, 2.F,
                    m_pCFft->getLength(CFft::kLengthMagnitude));
}
...
Error_t CChordIf::init_() {
    ...
    CChordFromBlockIf::create(
        m_pCChord, m_pCFft->getLength(CFft::kLengthMagnitude), m_fSampleRate);
    CMatrix::alloc(m_ppfChordProbs, kNumChords,
                   static_cast<int>(m_pCBlockAudio->getNumBlocks()));
...
void CChordIf::initViterbi_() {
    ...
    const int aiCircleOfFifths[24] = {0,  -5, 2,  -3, 4,  -1, 6,  1,
                                      -4, 3,  -2, 5,  -3, 4,  -1, 6,
                                      1,  -4, 3,  -2, 5,  0,  -5, 2};
    float fRadius = 1.F;
    float fDistance = .5F;
    ...
    for (auto m = 0; m < kNumChords - 1; m++) {
        float fX1 = static_cast<float>(
            fRadius * std::cos(2. * M_PI * aiCircleOfFifths[m] / 12.));
        float fY1 = static_cast<float>(
            fRadius * std::sin(2. * M_PI * aiCircleOfFifths[m] / 12.));
        float fZ1 = fDistance - ((m < 12) ? 0 : fDistance);
        for (auto n = 0; n < kNumChords - 1; n++) {
            float fXDist = static_cast<float>(
                fX1 -
                fRadius * std::cos(2. * M_PI * aiCircleOfFifths[n] / 12.));
            float fYDist = static_cast<float>(
                fY1 -
                fRadius * std::sin(2. * M_PI * aiCircleOfFifths[n] / 12.));
            float fZDist = fZ1 - (fDistance - ((n < 12) ? 0 : fDistance));

            ppfTransProb[m][n] =
                .1F +
                std::sqrt(fXDist * fXDist + fYDist * fYDist + fZDist * fZDist);
        }
    }
    // convert to probabilities
    CMatrix::mulC_I(
        ppfTransProb,
        -1.F / (.1F + CMatrix::getMax(ppfTransProb, kNumChords, kNumChords)),
        kNumChords, kNumChords);
    ...
    for (auto m = 0; m < kNumChords; m++)
        CVector::mulC_I(ppfTransProb[m],
                        1.F / CVector::getSum(ppfTransProb[m], kNumChords),
                        kNumChords);
