...
CNmf::CNmf(void) { reset(); }

CNmf::~CNmf(void) { reset(); }

Error_t CNmf::init(CNmfResult *pCNmfResult, int iRank, int iNumRows,
                   int iNumCols, int iMaxIter /*= 300*/,
                   float fSparsity /*= 0*/) {
    if (!pCNmfResult || iRank <= 0 || iNumRows <= 0 || iNumCols <= 0 ||
        iMaxIter <= 0)
...
Error_t CNmf::compNmf(CNmfResult *pCNmfResult,
                      const float *const *const ppfIn) {
    ...
    for (int k = 0; k < m_iMaxIter; k++) {
        ...
        if (k > 0) {
            if (std::abs((afCost[kCurrCost] - afCost[kPrevCost]) /
...
float CNmf::runNmfIter(CNmfResult *pCNmfResult,
                       const float *const *const ppfIn) {
    ...
    const bool bUpdateH = true, bUpdateW = true;
    ...
    if (bUpdateH) {
        ...
        // denominator
        CMatrix::mulMatMat(m_ppfDenom, m_ppfTransp, m_ppfOnes, iRank, iNumBins,
                           iNumBins, iNumObs);
        CMatrix::addC_I(m_ppfDenom, m_fSparsity + m_kMinOffset, iRank, iNumObs);

        // numerator
        CMatrix::mulMatMat(m_ppfNum, m_ppfTransp, m_ppfX, iRank, iNumBins,
                           iNumBins, iNumObs);
    ...
    if (bUpdateW) {
        ...
        // denominator
        CMatrix::mulMatMat(m_ppfDenom, m_ppfOnes, m_ppfTransp, iNumBins,
                           iNumObs, iNumObs, iRank);
        CMatrix::addC_I(m_ppfDenom, m_kMinOffset, iNumBins, iRank);

        // numerator
        CMatrix::mulMatMat(m_ppfNum, m_ppfX, m_ppfTransp, iNumBins, iNumObs,
                           iNumObs, iRank);
    ...
    // deal with sparsity
    CMatrix::addC_I(ppfXHat,
                    m_fSparsity * CMatrix::getNorm(ppfXHat, iNumBins, iNumObs),
                    iNumBins, iNumObs);
...
CNmfResult::~CNmfResult(void) { reset(); }
...
Error_t CNmfResult::init(int iRank, int iNumRows, int iNumCols) {
    ...
    for (auto m = 0; m < kNumMatrices; m++)
        CMatrix::alloc(m_appfMatrices[m], m_aaiMatrixDims[m][0],
                       m_aaiMatrixDims[m][1]);

    // init with random values
    CMatrix::setRand(m_appfMatrices[kW], m_aaiMatrixDims[kW][0],
                     m_aaiMatrixDims[kW][1]);
    CMatrix::setRand(m_appfMatrices[kH], m_aaiMatrixDims[kH][0],
                     m_aaiMatrixDims[kH][1]);

    // normalize
    CMatrix::vecnorm_I(m_appfMatrices[kW], m_aaiMatrixDims[kW][0],
                       m_aaiMatrixDims[kW][1]);
...
void CNmfResult::getMatDims(NmfMatrices_t eMatIdx, int &iNumRows,
                            int &iNumCols) const {
...
Error_t CNmfResult::getMat(float **ppfDest, NmfMatrices_t eMatIdx) {
    ...
    CMatrix::copy(ppfDest, m_appfMatrices[eMatIdx], m_aaiMatrixDims[eMatIdx][0],
                  m_aaiMatrixDims[eMatIdx][1]);

    return Error_t::kNoError;
}

bool CNmfResult::isInitialized() const { return m_bIsInitialized; }
