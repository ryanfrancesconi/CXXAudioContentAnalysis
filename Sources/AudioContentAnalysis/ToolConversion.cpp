...
float CConversion::convertFreq2Mel(float fInHz,
                                   MelConversionFunctions_t eFunc) {
    assert(fInHz >= 0);
...
void CConversion::convertFreq2Mel(float *pfMel, const float *pffInHz,
                                  int iLengthBuff,
                                  MelConversionFunctions_t eFunc) {
    assert(pfMel);
    assert(pffInHz);
...
void CConversion::convertMel2Freq(float *pffInHz, const float *pfMel,
                                  int iLengthBuff,
                                  MelConversionFunctions_t eFunc) {
    assert(pfMel);
    assert(pffInHz);
    ...
    for (auto k = 0; k < iLengthBuff; k++)
        pffInHz[k] =
            m_MelDispatchMap.at(eFunc + kNumMelConversionFunctions)(pfMel[k]);
}
...
float CConversion::convertFreq2Bark(float fInHz,
                                    BarkConversionFunctions_t eFunc) {
    assert(fInHz >= 0);
    return m_BarkDispatchMap.at(eFunc)(fInHz);
}

float CConversion::convertBark2Freq(float fBark,
                                    BarkConversionFunctions_t eFunc) {
    assert(fBark >= 0);
    return m_BarkDispatchMap.at(eFunc + kNumBarkConversionFunctions)(fBark);
}

void CConversion::convertFreq2Bark(float *pfBark, const float *pffInHz,
                                   int iLengthBuff,
                                   BarkConversionFunctions_t eFunc) {
    assert(pfBark);
    assert(pffInHz);
...
void CConversion::convertBark2Freq(float *pffInHz, const float *pfBark,
                                   int iLengthBuff,
                                   BarkConversionFunctions_t eFunc) {
    assert(pfBark);
    assert(pffInHz);
    ...
    for (auto k = 0; k < iLengthBuff; k++)
        pffInHz[k] = m_BarkDispatchMap.at(eFunc + kNumBarkConversionFunctions)(
            pfBark[k]);
}
...
float CConversion::convertFreq2BarkZwicker(float fFrequency) {
    assert(fFrequency > 0);
    return 13.F * std::atan(0.76F * fFrequency / 1000.F) +
           3.5F * std::atan(fFrequency / 7500.F);
}
float CConversion::convertBark2FreqZwicker(float /*fBark*/) { return -1.F; }
...
//////////////////////////////////////////////////////////////////////////////
void CConversion::convertBin2Freq(float *pffInHz, const float *pfBin,
