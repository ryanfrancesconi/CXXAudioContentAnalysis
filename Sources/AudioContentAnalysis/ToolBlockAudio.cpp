...
class CBlockAudioFile : public CBlockAudioIf {
  public:
    CBlockAudioFile(CAudioFileIf *pCAudioFile, int iBlockLength, int iHopLength)
        : m_pCAudioFile(pCAudioFile), m_pCRingBuff(0), m_ppfAudioData(0) {
        ...
        while (m_pCRingBuff->getNumValuesInBuffer() <
               m_pCRingBuff->getLength() - m_iHopLength - 1)
            readFile2RingBuff();
    }
    ...
    int getNextBlock(float *pfBlock, float *pfTimeStamp) override {
        ...
        if (iNumSamples < m_iBlockLength) {
            for (int c = 0; c < m_iNumChannels; c++)
                CVector::setZero(&pfBlock[iNumSamples],
                                 static_cast<long long>(m_iBlockLength) -
                                     iNumSamples);
  ...
  private:
    CBlockAudioFile(
        const CBlockAudioFile &that); //!< disallow copy construction
    CBlockAudioFile &operator=(const CBlockAudioFile &c);

    inline void readFile2RingBuff() {
        // set file read length variable
        // long long iNumSamples = std::min(m_iHopLength,
        // m_pCRingBuff->getLength() - m_pCRingBuff->getNumValuesInBuffer() +
        // 1);
        ...
        if (iNumSamples < m_iHopLength) {
            for (int c = 0; c < m_iNumChannels; c++)
                CVector::setZero(&m_ppfAudioData[c][iNumSamples],
                                 m_iHopLength - iNumSamples);

            iNumSamples = m_iHopLength;
        }

        // downmix in case of multichannel
        CPreProc::downmix(m_ppfAudioData[0], m_ppfAudioData, m_iNumChannels,
                          iNumSamples);
...
class CBlockAudioBuffer : public CBlockAudioIf {
  public:
    CBlockAudioBuffer(const float *pfAudioBuff, long long iAudioLength,
                      int iBlockLength, int iHopLength, float fSampleRate)
        : m_iCurrIdx(0), m_pfAudioData(0) {
    ...
    bool isEndOfData() const override { return m_iAudioLength == m_iCurrIdx; }

    int getNextBlock(float *pfBlock, float *pfTimeStamp) override {
        if (!m_pfAudioData)
            return -1;

        int iNumSamplesInBlock =
            m_iAudioLength - m_iCurrIdx < m_iBlockLength
                ? static_cast<int>(m_iAudioLength - m_iCurrIdx)
                : m_iBlockLength;

        CVector::copy(pfBlock, &m_pfAudioData[m_iCurrIdx], iNumSamplesInBlock);
        CVector::setZero(&pfBlock[iNumSamplesInBlock],
                         static_cast<long long>(m_iBlockLength) -
                             iNumSamplesInBlock);
  ...
  private:
    CBlockAudioBuffer(
        const CBlockAudioBuffer &that); //!< disallow copy construction
...
Error_t CBlockAudioIf::create(CBlockAudioIf *&pCInstance,
                              CAudioFileIf *pCAudioFile, int iBlockLength,
                              int iHopLength) {
    if (!pCAudioFile)
...
Error_t CBlockAudioIf::create(CBlockAudioIf *&pCInstance,
                              const float *pfAudioBuff, long long iAudioLength,
                              int iBlockLength, int iHopLength,
                              float fSampleRate) {
    if (!pfAudioBuff)
        return Error_t::kFunctionInvalidArgsError;
    if (iAudioLength <= 0 || iBlockLength <= 0 || iHopLength <= 0 ||
        iHopLength > iBlockLength || fSampleRate <= 0)
        return Error_t::kFunctionInvalidArgsError;

    pCInstance = new CBlockAudioBuffer(pfAudioBuff, iAudioLength, iBlockLength,
                                       iHopLength, fSampleRate);
