...
Error_t CFingerprint::init(const std::string &strAudioFilePath) {
    ...
    while (!pCAudioFile->isEof()) {
        ...
        CPreProc::downmix(&pfAllAudio[iCurrPos], ppfAudioBlock,
...
Error_t CFingerprint::init(const float *pfIn, long long iNumFrames,
                           float fSampleRate) {
    if (!pfIn)
        return Error_t::kFunctionInvalidArgsError;
...
Error_t CFingerprint::compFingerprint(uint32_t *puiFingerprint) {
    ...
    for (auto n = 0; n < iNumBlocks; n++) {
        ...
        puiFingerprint[n] =
            m_pCSubFingerprint->compSubFingerprint(m_pfProcBuff1);
    }
...
Error_t CFingerprint::init_() {
    ...
    CBlockAudioIf::create(m_pCBlockAudio, m_pfAudioBuff, m_iAudioLength,
                          m_iBlockLength, m_iHopLength, m_fProcSampleRate);
