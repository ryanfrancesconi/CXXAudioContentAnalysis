...
CCcf::CCcf(void) { m_pCFft = new CFft(); }
...
Error_t CCcf::compCcf(const float *pfIn1, const float *pfIn2, bool bNormalize) {
    ...
    if (bNormalize) {
        float fPower =
            std::sqrt(CVector::mulScalar(pfIn1, pfIn1, m_iBlockLength));
    ...
    CVector::mulC_I(m_apfData[1], static_cast<float>(m_iFftLength),
                    m_iFftLength);
    ...
    // copy results
    CVector::copy(m_apfData[1],
                  &m_apfData[0][static_cast<long long>(m_iFftLength) -
                                m_iBlockLength + 1],
                  static_cast<long long>(m_iBlockLength) - 1);
    CVector::copy(&m_apfData[1][m_iBlockLength - 1], m_apfData[0],
                  m_iBlockLength);
...
Error_t CCcf::getCcf(float *pfCcfResult, bool bIsAcf) const {
    ...
    CVector::copy(pfCcfResult, &m_apfData[1][iStartIdx],
                  static_cast<long long>(2) * m_iBlockLength - 1 - iStartIdx);
...
float CCcf::getCcfMax(bool bIsAcf) const {
    ...
    int iStartIdx = bIsAcf ? m_iBlockLength - 1 : 0;
    return CVector::getMax(&m_apfData[1][iStartIdx],
                           static_cast<long long>(2) * m_iBlockLength - 1 -
                               iStartIdx);
}
...
int CCcf::getCcfMaxIdx(bool bIsAcf) const {
    ...
    CVector::findMax(&m_apfData[1][iStartIdx], fMax, iMax,
                     static_cast<long long>(2) * m_iBlockLength - 1 -
                         iStartIdx);
