...
CKmeans::~CKmeans(void) { this->reset(); }
...
Error_t CKmeans::compKmeans(int *piResult,
    ...
    for (auto i = 0; i < m_iMaxIter; i++) {
        ...
        CMatrix::copy(m_appfClusterMeans[kPrev], m_appfClusterMeans[kCurr],
                      m_iK, m_iNumFeatures);
...
void CKmeans::compClusterMeans_(const float *const *const ppfFeatures,
                                const int *piResult) {
    ...
    for (auto k = 0; k < m_iK; k++) {
        if (!m_piClusterSize[k])
            continue;
        CVector::mulC_I(m_appfClusterMeans[kCurr][k], 1.F / m_piClusterSize[k],
                        m_iNumFeatures);
    }
}
...
bool CKmeans::checkConverged_() {
    ...
    while ((fSum += CVector::getSum(m_pfProc, m_iNumFeatures, true)) <= 0 &&
           k < m_iK - 1) {
        k++;
        CVector::copy(m_pfProc, m_appfClusterMeans[kCurr][k], m_iNumFeatures);
        CVector::sub_I(m_pfProc, m_appfClusterMeans[kPrev][k], m_iNumFeatures);
...
void CKmeans::assignClusterLabels_(int *piResult,
                                   const float *const *const ppfFeatures) {
    ...
    for (auto n = 0; n < m_iNumObs; n++) {
        ...
        for (auto k = 0; k < m_iK; k++)
            m_pfDist[k] = CVector::distEuclidean(m_appfClusterMeans[kCurr][k],
                                                 m_pfProc, m_iNumFeatures);
