...
Error_t CLeaveOneOutCrossVal::init(int iNumFeatures, int iNumObs,
                                   CClassifierBase *pCClassifier) {
    if (iNumFeatures <= 0 || iNumObs <= 1)
        return Error_t::kFunctionInvalidArgsError;
    if (!pCClassifier)
        return Error_t::kFunctionInvalidArgsError;

    if (m_iNumFeatures == iNumFeatures && m_iNumObs == iNumObs &&
        m_pCClassifier == pCClassifier) {
        m_pCClassifier->reset();
        CMatrix::setZero(m_ppfTrain, m_iNumFeatures,
                         m_iNumObs - static_cast<long long>(1));
        CVector::setZero(m_piClassLabels,
                         m_iNumObs - static_cast<long long>(1));
        ...
        CMatrix::alloc(m_ppfTrain, m_iNumFeatures,
                       m_iNumObs - static_cast<long long>(1));
...
float CLeaveOneOutCrossVal::process(const float *const *const ppfTrainFeatures,
    ...
    CMatrix::copy(m_ppfTrain, ppfTrainFeatures, m_iNumFeatures,
                  m_iNumObs - static_cast<long long>(1));
    CVector::copy(m_piClassLabels, piTrainClassIndices,
                  m_iNumObs - static_cast<long long>(1));

    for (int n = 0; n < m_iNumObs - 1; n++) {
        CMatrix::getCol(m_pfQuery, ppfTrainFeatures, m_iNumObs - 1 - n,
                        m_iNumFeatures);
        ...
        // prep for next round
        CMatrix::setCol(m_ppfTrain, m_pfQuery, m_iNumObs - 2 - n,
                        m_iNumFeatures);
