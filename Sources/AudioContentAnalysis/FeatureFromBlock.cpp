...
float CFeatureFromBlockIf::compFeatureSpectralCentroid(
    const float *pfMagSpec, int iDataLength, float fSampleRate /*= 1.F*/) {
    assert(pfMagSpec);
...
float CFeatureFromBlockIf::compFeatureSpectralCrestFactor(
    const float *pfMagSpec, int iDataLength, float /*fSampleRate = 1.F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
...
float CFeatureFromBlockIf::compFeatureSpectralDecrease(
    const float *pfMagSpec, int iDataLength, float /*fSampleRate = 1.F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
...
float CFeatureFromBlockIf::compFeatureSpectralFlatness(
    const float *pfMagSpec, int iDataLength, float /*fSampleRate = 1.F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
    ...
    if (fNorm < m_kfFloatThresh ||
        CVector::getMin(pfMagSpec, iDataLength) < m_kfFloatThresh)
        return 0;
...
float CFeatureFromBlockIf::compFeatureSpectralFlux(
    const float *pfMagSpec, const float *pfPrevSpec, int iDataLength,
...
float CFeatureFromBlockIf::compFeatureSpectralSpread(
    const float *pfMagSpec, int iDataLength, float fSampleRate /*= 1.F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
    assert(fSampleRate > 0);
    ...
    float fvss = 0;
    float fvsc =
        compFeatureSpectralCentroid(pfMagSpec, iDataLength, fSampleRate) * 2 /
        fSampleRate * (iDataLength - 1);
...
float CFeatureFromBlockIf::compFeatureSpectralKurtosis(
    const float *pfMagSpec, int iDataLength, float fSampleRate /*= 1.F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
    assert(fSampleRate > 0);
    ...
    float fvsc =
        ...
        fSampleRate * (iDataLength - 1);
    float fvss =
        compFeatureSpectralSpread(pfMagSpec, iDataLength, fSampleRate) * 2 /
        fSampleRate * (iDataLength - 1);
...
float CFeatureFromBlockIf::compFeatureSpectralRolloff(
    const float *pfMagSpec, int iDataLength, float fSampleRate /*= 1.F*/,
    float fKappa /*= .85F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
...
float CFeatureFromBlockIf::compFeatureSpectralSkewness(
    const float *pfMagSpec, int iDataLength, float fSampleRate /*= 1.F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
    assert(fSampleRate > 0);
...
float CFeatureFromBlockIf::compFeatureSpectralSlope(
    const float *pfMagSpec, int iDataLength, float fSampleRate /*= 1.F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
    assert(fSampleRate > 0);
...
float CFeatureFromBlockIf::compFeatureSpectralTonalPowerRatio(
    const float *pfMagSpec, int iDataLength, float /*fSampleRate = 1.F*/,
    float fThresh /*= 5e-4F*/) {
    assert(pfMagSpec);
    assert(iDataLength > 0);
    ...
    // initialize
    float fvtpr = 0;
    float fNorm = pfMagSpec[0] * pfMagSpec[0] +
                  pfMagSpec[iDataLength - 1] * pfMagSpec[iDataLength - 1];
    ...
    for (auto k = 1; k < iDataLength - 1; k++) {
        ...
        if (pfMagSpec[k] <= pfMagSpec[k - 1] ||
            pfMagSpec[k] <= pfMagSpec[k + 1] || pfMagSpec[k] <= fThresh)
            continue;
        else {
            fvtpr += fSquare;

            // increment because the next bin cannot be a local max but don't
            // forget fNorm
...
float CFeatureFromBlockIf::compFeatureTimeAcfCoeff(const float *pfSamples,
                                                   int iDataLength,
                                                   float /*fSampleRate = 1.F*/,
                                                   int iEta /*= 19*/) {
    assert(pfSamples);
    ...
    return CVector::mulScalar(pfSamples, &pfSamples[iEta],
                              static_cast<long long>(iDataLength) - iEta);
}

float CFeatureFromBlockIf::compFeatureTimePeakEnvelope(
    const float *pfSamples, int iDataLength, float /*fSampleRate = 1.F*/) {
    assert(pfSamples);
    assert(iDataLength > 0);
...
float CFeatureFromBlockIf::compFeatureTimeStd(const float *pfSamples,
                                              int iDataLength,
                                              float /*fSampleRate = 1.F*/) {
    assert(pfSamples);
    assert(iDataLength > 0);
...
float CFeatureFromBlockIf::compFeatureTimeRms(const float *pfSamples,
                                              int iDataLength,
                                              float /*fSampleRate = 1.F*/) {
    assert(pfSamples);
    assert(iDataLength > 0);
...
float CFeatureFromBlockIf::compFeatureTimeZeroCrossingRate(
    const float *pfSamples, int iDataLength, float /*fSampleRate = 1.F*/) {
    assert(pfSamples);
    assert(iDataLength > 0);
...
CFeatureFromBlockIf::CFeatureFromBlockIf(CFeatureIf::Feature_t eFeatureIdx,
                                         int iDataLength, float fSampleRate)
    : m_eFeatureIdx(eFeatureIdx), m_iDataLength(iDataLength),
...
class CFeatureSpectralFlux : public CFeatureFromBlockIf {
  public:
    CFeatureSpectralFlux(CFeatureIf::Feature_t eFeatureIdx, int iDataLength,
                         float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {
        CVector::alloc(m_pfPrevSpec, m_iDataLength);
        CVector::setZero(m_pfPrevSpec, m_iDataLength);
    };

    virtual ~CFeatureSpectralFlux() { CVector::free(m_pfPrevSpec); };

    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        *pfFeature = compFeatureSpectralFlux(pfIn, m_pfPrevSpec, m_iDataLength,
                                             m_fSampleRate);
  ...
  private:
    ...
    CFeatureSpectralFlux(
        const CFeatureSpectralFlux &that); //!< disallow copy construction
...
class CFeatureSpectralMfccs : public CFeatureFromBlockIf {
  public:
    CFeatureSpectralMfccs(CFeatureIf::Feature_t eFeatureIdx, int iDataLength,
                          float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {
    ...
    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        ...
        for (auto c = 0; c < m_iNumBands; c++)
            m_pfMelSpec[c] = std::log10(
                CVector::mulScalar(m_ppfH[c], pfIn, m_iDataLength) + 1e-20F);

        // compute dct
        for (auto j = 0; j < m_iNumMfcCoeffs; j++)
            pfFeature[j] =
                CVector::mulScalar(m_ppfDct[j], m_pfMelSpec, m_iNumBands);

        return Error_t::kNoError;
    };

    int getFeatureDimensions() const override { return m_iNumMfcCoeffs; }

    bool hasAdditionalParam() const override { return true; }
  ...
  private:
    ...
    CFeatureSpectralMfccs(
        const CFeatureSpectralMfccs &that); //!< disallow copy construction
    ...
    void genMfccFilters_() {
        ...
        const double dLinSpacing = 200. / 3.,
                     dLogSpacing = 1.0711703; // note sure where this mel
                                              // resolution comes from exactly

        assert(m_iNumBands > iNumLinFilters);
        double adBoundFreqs[3] = {dFreq, dFreq + dLinSpacing,
                                  dFreq + 2. * dLinSpacing};
        int aiBoundIdx[3] = {static_cast<int>(CConversion::convertFreq2Bin(
                                 static_cast<float>(adBoundFreqs[0]),
                                 (m_iDataLength - 1) * 2, m_fSampleRate)),
                             static_cast<int>(CConversion::convertFreq2Bin(
                                 static_cast<float>(adBoundFreqs[1]),
                                 (m_iDataLength - 1) * 2, m_fSampleRate)),
                             static_cast<int>(CConversion::convertFreq2Bin(
                                 static_cast<float>(adBoundFreqs[2]),
                                 (m_iDataLength - 1) * 2, m_fSampleRate))};
        ...
        for (auto c = 0; c < m_iNumBands; c++) {
            ...
            for (auto k = aiBoundIdx[0]; k <= aiBoundIdx[1]; k++) {
                float fFreqk = CConversion::convertBin2Freq(
                    1.F * k, (m_iDataLength - 1) * 2, m_fSampleRate);
                if ((fFreqk - adBoundFreqs[0]) <= 0.F)
                    continue;
                m_ppfH[c][k] =
                    static_cast<float>(dFilterAmp * (fFreqk - adBoundFreqs[0]) /
                                       (adBoundFreqs[1] - adBoundFreqs[0]));
                assert(m_ppfH[c][k] >= 0);
            }
            ...
            for (auto k = aiBoundIdx[1] + 1; k <= aiBoundIdx[2]; k++) {
                float fFreqk = CConversion::convertBin2Freq(
                    1.F * k, (m_iDataLength - 1) * 2, m_fSampleRate);
                m_ppfH[c][k] =
                    static_cast<float>(dFilterAmp * (adBoundFreqs[2] - fFreqk) /
                                       (adBoundFreqs[2] - adBoundFreqs[1]));
                assert(m_ppfH[c][k] >= 0);
            }
            ...
            adBoundFreqs[2] = (c < iNumLinFilters - 3)
                                  ? adBoundFreqs[1] + dLinSpacing
                                  : adBoundFreqs[2] * dLogSpacing; //!< Check me
            aiBoundIdx[0] = aiBoundIdx[1];
            aiBoundIdx[1] = aiBoundIdx[2];
            aiBoundIdx[2] = static_cast<int>(CConversion::convertFreq2Bin(
                static_cast<float>(adBoundFreqs[2]), (m_iDataLength - 1) * 2,
                m_fSampleRate));
        }
    }
    ...
    void genDctMat_(int iNumCoeffs) {
        ...
        for (auto c = 0; c < iNumCoeffs; c++) {
            for (auto b = 0; b < m_iNumBands; b++)
                m_ppfDct[c][b] = static_cast<float>(
                    std::cos(c * (2. * b + 1) * M_PI / 2. / m_iNumBands));

            CVector::mulC_I(m_ppfDct[c], 1 / std::sqrt(m_iNumBands / 2.F),
                            m_iNumBands);
        }
    ...
    void deleteDctMat_() { CMatrix::free(m_ppfDct, m_iNumMfcCoeffs); }
...
class CFeatureSpectralPitchChroma : public CFeatureFromBlockIf {
  public:
    CFeatureSpectralPitchChroma(CFeatureIf::Feature_t eFeatureIdx,
                                int iDataLength, float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {
        // alloc transfer function memory
    ...
    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        ...
        for (auto p = 0; p < m_iNumPitchClasses; p++) {
            // we could do this nicer with CVector::mulScalar if we allocated
            // memory
    ...
    int getFeatureDimensions() const override { return m_iNumPitchClasses; }
  ...
  private:
    ...
    CFeatureSpectralPitchChroma(const CFeatureSpectralPitchChroma
                                    &that); //!< disallow copy construction
    CFeatureSpectralPitchChroma &
    operator=(const CFeatureSpectralPitchChroma &c);

    void genPcFilters_() {
        const float fStartPitch = 60; //!< C4
        float fMid = CConversion::convertMidi2Freq(fStartPitch, m_fA4);

        // sanity check: reduce number of octaves if highest freq is higher than
        // half fs
        ...
        for (auto p = 0; p < m_iNumPitchClasses; p++) {
            ...
            for (auto o = 0; o < m_iNumOctaves; o++) {
                ...
                const int aiBoundIdx[2] = {
                    static_cast<int>(CConversion::convertFreq2Bin(
                        afBoundFreqs[0], (m_iDataLength - 1) * 2,
                        m_fSampleRate)) +
                        1,
                    static_cast<int>(CConversion::convertFreq2Bin(
                        afBoundFreqs[1], (m_iDataLength - 1) * 2,
                        m_fSampleRate))};

                // set transfer function
                CVector::setValue(&m_ppfH[p][aiBoundIdx[0]],
                                  1.F / (aiBoundIdx[1] - aiBoundIdx[0] + 1),
                                  static_cast<long long>(aiBoundIdx[1]) -
                                      aiBoundIdx[0] + 1);
...
class CFeatureSpectralRolloff : public CFeatureFromBlockIf {
  public:
    CFeatureSpectralRolloff(CFeatureIf::Feature_t eFeatureIdx, int iDataLength,
                            float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {};
    ...
    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        *pfFeature = compFeatureSpectralRolloff(pfIn, m_iDataLength,
                                                m_fSampleRate, m_fKappa);
...
class CFeatureSpectralTonalPowerRatio : public CFeatureFromBlockIf {
  public:
    CFeatureSpectralTonalPowerRatio(CFeatureIf::Feature_t eFeatureIdx,
                                    int iDataLength, float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {};
    ...
    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        *pfFeature = compFeatureSpectralTonalPowerRatio(
            pfIn, m_iDataLength, m_fSampleRate, m_fThresh);
...
class CFeatureTimeAcfCoeff : public CFeatureFromBlockIf {
  public:
    CFeatureTimeAcfCoeff(CFeatureIf::Feature_t eFeatureIdx, int iDataLength,
                         float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {};
    ...
    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        *pfFeature =
            compFeatureTimeAcfCoeff(pfIn, m_iDataLength, m_fSampleRate, m_iEta);
  ...
  private:
    ...
    int m_iEta =
        19; //!< int this example implementation, we chose the 19th coefficient
};
...
class CFeatureTimeMaxAcf : public CFeatureFromBlockIf {
  public:
    CFeatureTimeMaxAcf(CFeatureIf::Feature_t eFeatureIdx, int iDataLength,
                       float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {
        m_pCCcf = new CCcf();
    ...
    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        ...
        int iEta = 0, iEtaMin = static_cast<int>(m_fSampleRate / m_fMax);
        ...
        *pfFeature = CVector::getMax(
            &m_pfAcf[iEtaMin], static_cast<long long>(m_iDataLength) - iEtaMin,
            true);
  ...
  private:
    CFeatureTimeMaxAcf() {};
    CFeatureTimeMaxAcf(
        const CFeatureTimeMaxAcf &that); //!< disallow copy construction
...
class CFeatureTimePeakEnvelope : public CFeatureFromBlockIf {
  public:
    CFeatureTimePeakEnvelope(CFeatureIf::Feature_t eFeatureIdx, int iDataLength,
                             float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {
        for (auto i = 0; i < kNumPpmFilters; i++)
            m_afAlpha[i] = CSinglePoleLp::calcFilterParam(
                m_afIntegrationTimeInS[i], fSampleRate);
    };
    ...
    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        ...
        pfFeature[kBlockMax] =
            compFeatureTimePeakEnvelope(pfIn, m_iDataLength, m_fSampleRate);
    ...
    int getFeatureDimensions() const override { return kNumPeakTypes; }
  ...
  private:
    ...
    float ppm_I(float fIn) {
        ...
        if (m_fFilterBuff > fIn)
            ...
            fOut = (1.F - m_afAlpha[kAttack]) * fIn +
                   m_afAlpha[kAttack] * m_fFilterBuff;
    ...
    const float m_afIntegrationTimeInS[kNumPpmFilters] = {
        .01F, 1.5F}; //!< integration times for filter initialization
...
class CFeatureTimeRms : public CFeatureFromBlockIf {
  public:
    CFeatureTimeRms(CFeatureIf::Feature_t eFeatureIdx, int iDataLength,
                    float fSampleRate)
        : CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate) {
        CSinglePoleLp::create(m_pCSinglePole);
        m_pCSinglePole->setFilterParam(
            CSinglePoleLp::calcFilterParam(m_fIntegrationTimeInS, fSampleRate));
    };

    virtual ~CFeatureTimeRms() { CSinglePoleLp::destroy(m_pCSinglePole); };

    Error_t compFeature(float *pfFeature, const float *pfIn) override {
        // extract 1st variant: rms per block
        pfFeature[kBlockRms] =
            compFeatureTimeRms(pfIn, m_iDataLength, m_fSampleRate);

        // extract 2nd variant: rms from single pole
        // do inefficient sample based processing so we don't have to alloc
        // extra memory
    ...
    int getFeatureDimensions() const override { return kNumRmsTypes; }
  ...
  private:
    ...
    CFeatureTimeRms() {};
    CFeatureTimeRms(
        const CFeatureTimeRms &that); //!< disallow copy construction

    CSinglePoleLp *m_pCSinglePole = 0; //!< instance of low pass filter

    float m_fIntegrationTimeInS =
        .3F; //!< integration time for filter coeff calculation
};

///////////////////////////////////////////////////////////////////
// normal member functions
Error_t CFeatureFromBlockIf::create(CFeatureFromBlockIf *&pCInstance,
                                    CFeatureIf::Feature_t eFeatureIdx,
                                    int iDataLength, float fSampleRate) {
    ...
    case CFeatureIf::kFeatureTimeZeroCrossingRate:
        pCInstance =
            new CFeatureFromBlockIf(eFeatureIdx, iDataLength, fSampleRate);
        break;

    case CFeatureIf::kFeatureSpectralFlux:
        pCInstance =
            new CFeatureSpectralFlux(eFeatureIdx, iDataLength, fSampleRate);
        break;

    case CFeatureIf::kFeatureSpectralMfccs:
        pCInstance =
            new CFeatureSpectralMfccs(eFeatureIdx, iDataLength, fSampleRate);
        break;

    case CFeatureIf::kFeatureSpectralPitchChroma:
        pCInstance = new CFeatureSpectralPitchChroma(eFeatureIdx, iDataLength,
                                                     fSampleRate);
        break;

    case CFeatureIf::kFeatureSpectralRolloff:
        pCInstance =
            new CFeatureSpectralRolloff(eFeatureIdx, iDataLength, fSampleRate);
        break;

    case CFeatureIf::kFeatureSpectralTonalPowerRatio:
        pCInstance = new CFeatureSpectralTonalPowerRatio(
            eFeatureIdx, iDataLength, fSampleRate);
        break;

    case CFeatureIf::kFeatureTimeAcfCoeff:
        pCInstance =
            new CFeatureTimeAcfCoeff(eFeatureIdx, iDataLength, fSampleRate);
        break;

    case CFeatureIf::kFeatureTimeMaxAcf:
        pCInstance =
            new CFeatureTimeMaxAcf(eFeatureIdx, iDataLength, fSampleRate);
        break;

    case CFeatureIf::kFeatureTimePeakEnvelope:
        pCInstance =
            new CFeatureTimePeakEnvelope(eFeatureIdx, iDataLength, fSampleRate);
        break;
...
Error_t CFeatureFromBlockIf::compFeature(float *pfFeature, const float *pfIn) {
    ...
    *pfFeature =
        m_DispatchMap.at(m_eFeatureIdx)(pfIn, m_iDataLength, m_fSampleRate);

    return Error_t::kNoError;
}

bool CFeatureFromBlockIf::hasAdditionalParam() const {
    // default: feature doesn't need any additional parameter (can be
    // overridden)
    return false;
}
