...
inline CFft::~CFft() { reset(); }

Error_t CFft::init(int iBlockLength, int iZeroPadFactor,
    ...
    int iInternalBlockLength = CUtil::isPowOf2(iBlockLength)
                                   ? iBlockLength
                                   : CUtil::nextPowOf2(iBlockLength);
...
Error_t CFft::compFft(complex_t *pfSpectrum, const float *pfIn) {
    ...
    CVector::setZero(&m_pfProcBuff[m_iDataLength],
                     static_cast<long long>(m_iFftLength) - m_iDataLength);
...
Error_t CFft::getMagnitude(float *pfMag, const complex_t *pfSpectrum) const {
    ...
    for (int i = 1; i < iNyq; i++) {
        ...
        pfMag[i] = sqrtf(pfSpectrum[i] * pfSpectrum[i] +
                         pfSpectrum[iImagIdx] * pfSpectrum[iImagIdx]);
    }
    return Error_t::kNoError;
}
...
Error_t CFft::splitRealImag(float *pfReal, float *pfImag,
...
Error_t CFft::mergeRealImag(complex_t *pfSpectrum, const float *pfReal,
...
void CFft::conjugate_I(complex_t *pfFftResult) const {
    // re(0),re(1),re(2),...,re(size/2),im(size/2-1),...,im(1)
    CVector::mulC_I(&pfFftResult[(m_iFftLength >> 1) + 1], -1.F,
                    static_cast<long long>(m_iFftLength >> 1) - 1);
}

void CFft::multiply_I(complex_t *pfFftSrc1Dest,
                      const complex_t *pfFftSrc2) const {
    int i = 0, j = 0;
...
Error_t CFft::computeWindow_(WindowFunction_t eWindow) {
    ...
    case kWindowHann: {
        for (i = 0; i < m_iDataLength; i++) {
            m_pfWindowBuff[i] =
                .5F * (1.F - cosf((i * 2.F * m_Pi) / (m_iDataLength + 1)));
        }
        break;
    }
    case kWindowHamming: {
        for (i = 0; i < m_iDataLength; i++) {
            m_pfWindowBuff[i] =
                .54F - .46F * cosf((i * 2.F * m_Pi) / (m_iDataLength + 1));
        }
        break;
    }
    default:
