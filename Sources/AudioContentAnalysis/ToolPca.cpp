...
Error_t CPca::compPca(float **ppfRes, float *pfEigenValues,
    ...
    calcSVD(m_ppfU, m_ppfW, m_ppfV, m_ppfProcTmp, m_iNumFeatures,
            m_iNumFeatures);
    ...
    // compute components
    CMatrix::mulMatMat(ppfRes, m_ppfProcTmp, ppfIn, m_iNumFeatures,
                       m_iNumFeatures, m_iNumFeatures, m_iNumObs);
...
Error_t CPca::compCov(float **ppfCovOut, const float *const *const ppfIn,
    ...
    for (auto m = 0; m < iNumRows; m++) {
        ...
        for (auto n = 0; n <= m; n++) {
            ...
            for (auto j = 0; j < iNumCols; j++)
                ppfCovOut[m][n] +=
                    (ppfIn[m][j] - fMean1) * (ppfIn[n][j] - fMean2);
...
Error_t CPca::calcSVD(float **ppfU, float **ppfW, float **ppfV,
                      const float *const *const ppfMat, int iNumRows,
                      int iNumCols, int iMaxIterations) {

    int i, k, l = 0;

    float *pdRV1;
    float dTmp, dF, dH, dS, dNorm = 0, dG = 0, dScale = 0;
    ...
    for (k = iNumCols - 1; k >= 0; k--) {
        for (int iIter = 0; iIter < iMaxIterations; iIter++) {
            ...
            dG = pdRV1[iNM];
            dH = pdRV1[k];
            dF = ((dY - dZ) * (dY + dZ) + (dG - dH) * (dG + dH)) /
                 (2.F * dH * dY);
            dG = matPythag(dF, 1.);
            dF = ((dX - dZ) * (dX + dZ) +
                  dH * ((dY / (dF + (dF >= 0 ? fabs(dG) : -fabs(dG)))) - dH)) /
                 dX;
            dC = 1;
            dS = 1;
