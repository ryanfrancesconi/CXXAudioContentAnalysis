...
/*! \brief class for computation of the pitch via the acf of the magnitude
 * spectrum
 */
class CPitchSpectralAcf : public CPitchFromBlockIf {
  public:
    CPitchSpectralAcf(CPitchIf::PitchExtractors_t ePitchIdx, int iDataLength,
                      float fSampleRate)
        : CPitchFromBlockIf(ePitchIdx, iDataLength, fSampleRate) {
        m_pCCcf = new CCcf();
        m_pCCcf->init((m_iDataLength - 1) * 2);
        CVector::alloc(m_pfProcBuff,
                       (static_cast<long long>(m_iDataLength) - 1) * 2);
    ...
    float compF0(const float *pfIn) override {
        ...
        // get minimum bin
        int iMin = CUtil::float2int<int>(CConversion::convertFreq2Bin(
            m_fMin, (m_iDataLength - 1) * 2, m_fSampleRate));
        float fTmp = 0;
        long long iMaxIdx = -1;
        ...
        CVector::copy(&m_pfProcBuff[m_iDataLength - 1], pfIn,
                      m_iDataLength - static_cast<long long>(1));
        ...
        CVector::findMax(&m_pfAcf[iMin], fTmp, iMaxIdx,
                         m_iDataLength - static_cast<long long>(1) - iMin);

        return CConversion::convertBin2Freq(
            (iMin + iMaxIdx) * 1.F, (m_iDataLength - 1) * 2, m_fSampleRate);
    };

  private:
    CPitchSpectralAcf() {};
    CPitchSpectralAcf(
        const CPitchSpectralAcf &that); //!< disallow copy construction
...
/*! \brief class for computation of the pitch via the hps of the magnitude
 * spectrum
 */
class CPitchSpectralHps : public CPitchFromBlockIf {
  public:
    CPitchSpectralHps(CPitchIf::PitchExtractors_t ePitchIdx, int iDataLength,
                      float fSampleRate)
        : CPitchFromBlockIf(ePitchIdx, iDataLength, fSampleRate) {
        CVector::alloc(m_pfProcBuff, m_iDataLength);
    }

    virtual ~CPitchSpectralHps() { CVector::free(m_pfProcBuff); }
    ...
    float compF0(const float *pfIn) override {
        ...
        CVector::setZero(&m_pfProcBuff[m_iDataLength / m_iOrder],
                         static_cast<long long>(m_iDataLength) -
                             (m_iDataLength / m_iOrder));
        ...
        return CConversion::convertBin2Freq(
            iMaxIdx * 1.F, (m_iDataLength - 1) * 2, m_fSampleRate);
    };

  private:
    CPitchSpectralHps() {};
    CPitchSpectralHps(
        const CPitchSpectralHps &that); //!< disallow copy construction
...
/*! \brief class for computation of the pitch via the acf of the time domain
 * signal
 */
class CPitchTimeAcf : public CPitchFromBlockIf {
  public:
    CPitchTimeAcf(CPitchIf::PitchExtractors_t ePitchIdx, int iDataLength,
                  float fSampleRate)
        : CPitchFromBlockIf(ePitchIdx, iDataLength, fSampleRate) {
        m_pCCcf = new CCcf();
    ...
    float compF0(const float *pfIn) override {
        ...
        float fMax = 0;
        long long iMax = -1;
        CVector::findMax(&m_pfAcf[iEtaMin], fMax, iMax,
                         static_cast<long long>(m_iDataLength) - iEtaMin);
...
/*! \brief class for computation of the pitch via the auditory method from the
 * time domain signal
 */
class CPitchTimeAuditory : public CPitchFromBlockIf {
  public:
    CPitchTimeAuditory(CPitchIf::PitchExtractors_t ePitchIdx, int iDataLength,
                       float fSampleRate)
        : CPitchFromBlockIf(ePitchIdx, iDataLength, fSampleRate) {
        float afA[3] = {0};
        float afB[3] = {0};
    ...
    float compF0(const float *pfIn) override {
        ...
        for (auto c = 0; c < m_iNumBands; c++) {
            // smooth
            m_apCFilter[c]->process(m_ppfProcBuff[c], m_ppfProcBuff[c],
                                    m_iDataLength);
  ...
  private:
    CPitchTimeAuditory() {};
    CPitchTimeAuditory(
        const CPitchTimeAuditory &that); //!< disallow copy construction
    ...
    int getAcfMax_(const float *pfInput) {
        ...
        float fMax = 0;
        long long iMax = -1;
        CVector::findMax(&pfInput[iEtaMin], fMax, iMax,
                         static_cast<long long>(m_iDataLength) - iEtaMin);
    ...
    const float m_fSmoothLpCutoff =
        0.02F; //!< cutoff frequency for smoothing lowpass
    const float m_fMinThresh = 0.35F; //!< minimum threshold for acf parsing
};

/*! \brief class for computation of the pitch via the amdf method from the time
 * domain signal
 */
class CPitchTimeAmdf : public CPitchFromBlockIf {
  public:
    CPitchTimeAmdf(CPitchIf::PitchExtractors_t ePitchIdx, int iDataLength,
                   float fSampleRate)
        : CPitchFromBlockIf(ePitchIdx, iDataLength, fSampleRate) {};
...
/*! \brief class for computation of the pitch via the zero crossings method from
 * the time domain signal
 */
class CPitchTimeZeroCrossings : public CPitchFromBlockIf {
  public:
    CPitchTimeZeroCrossings(CPitchIf::PitchExtractors_t ePitchIdx,
  ...
  private:
    ...
    CPitchTimeZeroCrossings(
        const CPitchTimeZeroCrossings &that); //!< disallow copy construction
    CPitchTimeZeroCrossings &operator=(const CPitchTimeZeroCrossings &c);
};

///////////////////////////////////////////////////////////////////
// normal member functions
Error_t CPitchFromBlockIf::create(CPitchFromBlockIf *&pCInstance,
                                  CPitchIf::PitchExtractors_t ePitchIdx,
                                  int iDataLength, float fSampleRate) {
    ...
    case CPitchIf::kPitchTimeZeroCrossings:
        pCInstance =
            new CPitchTimeZeroCrossings(ePitchIdx, iDataLength, fSampleRate);
        break;
...
CPitchFromBlockIf::CPitchFromBlockIf(CPitchIf::PitchExtractors_t ePitchIdx,
                                     int iDataLength, float fSampleRate)
    : m_ePitchIdx(ePitchIdx), m_iDataLength(iDataLength),
