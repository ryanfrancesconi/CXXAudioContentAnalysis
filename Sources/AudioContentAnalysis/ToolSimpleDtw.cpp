...
Error_t CDtw::compDtw(const float *const *const ppfDistanceMatrix) {
    ...
    float fFirstColCost =
        0; //!< variable that will only contain the cost of the first column
           //!< (for the current row to be processed)
    ...
    for (int j = 1; j < m_aiMatrixDims[kCol]; j++) {
        m_apfCost[kRowCurr][j] =
            m_apfCost[kRowCurr][j - 1] + ppfDistanceMatrix[0][j];
        m_ppePathIdx[0][j] = kHoriz;
    }
    for (int i = 1; i < m_aiMatrixDims[kRow]; i++)
        m_ppePathIdx[i][0] = kVert;

    // compute cost 'matrix' (we only use two rows here) and store backtracking
    // path
    for (int i = 1; i < m_aiMatrixDims[kRow]; i++) {
        // init the variables we need for the current row (remember the cost in
        // the first column, and then increment it)
        m_apfCost[kRowCurr][0] = fFirstColCost;
        fFirstColCost += ppfDistanceMatrix[i][0];
        m_apfCost[kRowNext][0] = fFirstColCost;

        for (int j = 1; j < m_aiMatrixDims[kCol]; j++) {
            m_ppePathIdx[i][j] = static_cast<unsigned char>(
                findMin_(m_apfCost[kRowNext][j - 1], // horiz
...
Error_t CDtw::getPath(int **ppiPathResult) const {
    ...
    // traceback
    while (iIdx > 0) {
        ppiPathResult[kRow][iIdx - 1] =
            ppiPathResult[kRow][iIdx] +
            aiDecrement[m_ppePathIdx[ppiPathResult[kRow][iIdx]]
                                    [ppiPathResult[kCol][iIdx]]][kRow];
        ppiPathResult[kCol][iIdx - 1] =
            ppiPathResult[kCol][iIdx] +
            aiDecrement[m_ppePathIdx[ppiPathResult[kRow][iIdx]]
                                    [ppiPathResult[kCol][iIdx]]][kCol];
        iIdx--;
    }
