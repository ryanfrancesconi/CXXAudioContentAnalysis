...
Error_t CKnn::train(const float *const *const ppfTrainFeatures,
    ...
    // note we store this transposed - that's easier for the distance
    // computation
...
int CKnn::getParamK() const { return m_iK; }
...
int CKnn::classify(const float *pfQuery) {
    ...
    for (auto n = 0; n < m_iNumObs; n++)
        m_pfSortDist[n] =
            CVector::distEuclidean(pfQuery, m_ppfTrain[n], m_iNumFeatures);
...
void CKnn::buildHistogram_(bool bUseDistance) {
    ...
    if (bUseDistance) {
        ...
        for (auto k = 0; k < iNumLabelsInK; k++)
            if (m_piHistCount[k] > 0)
                m_pfHist[k] /= m_piHistCount[k];
