...
class CFeatureFromFile : public CFeatureIf {
  public:
    CFeatureFromFile(Feature_t eFeatureIdx, std::string strAudioFilePath,
...
CFeatureFromFile::CFeatureFromFile(Feature_t eFeatureIdx,
...
class CFeatureFromVector : public CFeatureIf {
  public:
    CFeatureFromVector(Feature_t eFeatureIdx, const float *pfAudio,
                       long long iAudioLength, float fSampleRate,
                       int iBlockLength, int iHopLength);
    virtual ~CFeatureFromVector() {};
};

CFeatureFromVector::CFeatureFromVector(Feature_t eFeatureIdx,
                                       const float *pfAudio,
...
/////////////////////////////////////////////////////////////////////////////////
// base class
CFeatureIf::CFeatureIf() { reset_(); }

inline CFeatureIf::~CFeatureIf() { reset_(); }

Error_t CFeatureIf::create(CFeatureIf *&pCInstance, Feature_t eFeatureIdx,
    ...
    pCInstance = new CFeatureFromFile(eFeatureIdx, strAudioFilePath,
...
Error_t CFeatureIf::create(CFeatureIf *&pCInstance, Feature_t eFeatureIdx,
    ...
    pCInstance = new CFeatureFromVector(eFeatureIdx, pfAudio, iNumSamples,
...
CFeatureIf::Feature_t
CFeatureIf::getFeatureIdxFromString(std::string sFeatureString) {
...
Error_t CFeatureIf::init_(Feature_t eFeatureIdx) {
    if (isFeatureSpectral_(eFeatureIdx)) {
        ...
        CFeatureFromBlockIf::create(m_pCFeature, eFeatureIdx,
                                    m_pCFft->getLength(CFft::kLengthMagnitude),
                                    m_fSampleRate);
    } else {
        // allocate processing memory
        CVector::alloc(m_pfProcBuff1, m_iBlockLength);
        CFeatureFromBlockIf::create(m_pCFeature, eFeatureIdx, m_iBlockLength,
                                    m_fSampleRate);
    }
