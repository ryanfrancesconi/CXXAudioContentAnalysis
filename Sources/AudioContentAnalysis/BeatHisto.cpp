...
inline CBeatHistoIf::~CBeatHistoIf() { reset_(); }

Error_t CBeatHistoIf::create(CBeatHistoIf *&pCInstance,
...
Error_t CBeatHistoIf::create(CBeatHistoIf *&pCInstance, const float *pfAudio,
...
Error_t CBeatHistoIf::getNumBins(int &iBeatHistoLength,
                                 BeatHisto_t eBeatHistoComp) const {
    if (!m_bIsInitialized) {
        iBeatHistoLength = 0;
...
Error_t CBeatHistoIf::getBeatHistoAxisTicks(float *pfAxisTicks,
                                            BeatHisto_t eBeatHistoComp) const {
    if (!m_bIsInitialized) {
        return Error_t::kFunctionIllegalCallError;
    }
    ...
    if (eBeatHistoComp == kBeatHistoFft) {
        for (auto k = aiRangeIndices[0], j = 0; k <= aiRangeIndices[1];
             k++, j++)
            pfAxisTicks[j] = 60.F * CConversion::convertBin2Freq(
                                        1.F * k, m_iBeatHistoLength << 1,
                                        m_fSampleRate / m_iHopLength);
    } else {
        for (auto k = aiRangeIndices[0], j = 0; k <= aiRangeIndices[1];
             k++, j++)
            pfAxisTicks[j] =
                60.F / m_iHopLength * m_fSampleRate / (m_iBeatHistoLength - k);
    }

    return Error_t::kNoError;
}

Error_t CBeatHistoIf::compBeatHisto(float *pfBeatHisto,
                                    BeatHisto_t eBeatHistoComp) {
    if (!m_bIsInitialized)
        return Error_t::kFunctionIllegalCallError;
    if (!pfBeatHisto)
        return Error_t::kFunctionInvalidArgsError;
    ...
    CBlockAudioIf::create(pCBlock, m_pfNovelty, m_pCNovelty->getNumBlocks(),
                          m_iBeatHistoLength, iNoveltyHop,
                          m_fSampleRate / m_iHopLength);
    ...
    // copy to output
    CVector::copy(pfBeatHisto, &m_pfBeatHisto[aiBeatHistoRange[0]],
                  iBeatHistoLength);
...
Error_t CBeatHistoIf::init_(const std::string &strAudioFilePath) {
    if (Error_t::kNoError ==
        CNoveltyIf::create(m_pCNovelty, CNoveltyIf::kNoveltyFlux,
                           strAudioFilePath, m_iBlockLength, m_iHopLength)) {
        int iNumBlocks = 0;
        ...
        CVector::alloc(m_pfProcBuff,
                       static_cast<long long>(m_iBeatHistoLength) * 2);

        CVector::alloc(m_pfBeatHisto,
                       static_cast<long long>(m_iBeatHistoLength) + 1);
...
Error_t CBeatHistoIf::init_(const float *pfAudio, long long iNumSamples,
                            float fSampleRate) {
    if (Error_t::kNoError ==
        CNoveltyIf::create(m_pCNovelty, CNoveltyIf::kNoveltyFlux, pfAudio,
                           iNumSamples, fSampleRate, m_iBlockLength,
                           m_iHopLength)) {
        int iNumBlocks = 0;
        m_pCNovelty->getNumBlocks(iNumBlocks);
        assert(iNumBlocks > 2);
...
inline void CBeatHistoIf::compHistoRange_(int &iStartIdx, int &iStopIdx,
                                          BeatHisto_t eBeatHistoComp) const {
    const float afBpmRange[2] = {30.F, 200.F};

    if (eBeatHistoComp == kBeatHistoFft) {
        iStartIdx = static_cast<int>(CConversion::convertFreq2Bin(
            afBpmRange[0] / 60.F, m_iBeatHistoLength << 1,
            m_fSampleRate / m_iHopLength));
        iStopIdx = static_cast<int>(CConversion::convertFreq2Bin(
                       afBpmRange[1] / 60.F, m_iBeatHistoLength << 1,
                       m_fSampleRate / m_iHopLength)) +
                   1;
    } else {
        iStartIdx =
            m_iBeatHistoLength - static_cast<int>(60.F * m_fSampleRate /
                                                  m_iHopLength / afBpmRange[0]);
        iStopIdx =
            m_iBeatHistoLength - static_cast<int>(60.F * m_fSampleRate /
                                                  m_iHopLength / afBpmRange[1]);
    }
}
