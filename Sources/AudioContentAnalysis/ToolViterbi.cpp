...
CViterbi::CViterbi(void) { reset(); }

CViterbi::~CViterbi(void) { reset(); }

Error_t CViterbi::init(const float *const *const ppfPTransition,
...
Error_t CViterbi::compViterbi(const float *const *const ppfPEmission,
...
void CViterbi::compLogLikelihood_(const float *const *const ppfPEmission) {
    ...
    // initialize
    for (auto m = 0; m < m_iNumStates; m++)
        m_ppfProb[m][0] = std::log(ppfPEmission[m][0] + m_kLogMin) +
                          std::log(m_pfStart[m] + m_kLogMin);
...
float CViterbi::getOverallProbability() const { return m_fOverallProb; }
