...
CKey::CKey() { m_pCKeyFromChroma = new CKeyFromChroma(); }
...
Error_t CKey::init(const std::string &strAudioFilePath, int iBlockLength,
                   int iHopLength) {
    ...
    Error_t rErr = CFeatureIf::create(
        m_pCPitchChromaExtractor, CFeatureIf::kFeatureSpectralPitchChroma,
        strAudioFilePath, iBlockLength, iHopLength);
...
Error_t CKey::init(const float *pfAudio, long long iNumSamples,
    ...
    Error_t rErr = CFeatureIf::create(
        m_pCPitchChromaExtractor, CFeatureIf::kFeatureSpectralPitchChroma,
        pfAudio, iNumSamples, fSampleRate, iBlockLength, iHopLength);
...
int CKey::compKey() {
    ...
    for (auto n = 0; n < aiDim[0]; n++)
        afAvgPitchChroma[n] =
            CVector::getMean(m_ppfPitchChromagram[n], aiDim[1]);
