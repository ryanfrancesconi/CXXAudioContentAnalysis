...
Error_t CGmmClassifier::train(const float *const *const ppfTrainFeatures,
    ...
    // allocate GMM stuff but keep result for later (avoid mem leaks when
    // repeatedly calling train)
    if (m_ppCGmmResult) {
    ...
    for (auto c = 0; c < m_iNumClasses; c++) {
        ...
        for (auto n = 0; n < m_iNumObs; n++) {
            ...
            if (piTrainClassIndices[n] == m_piClassLabels[c]) {
                ...
                CMatrix::setCol(ppfTrain, m_pfQuery, iNumClassObs,
                                m_iNumFeatures);
                iNumClassObs++;
            }
        }
...
int CGmmClassifier::getNumMixtures() const { return m_iK; }
