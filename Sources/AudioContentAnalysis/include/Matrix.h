...
/*! \brief class with static functions for matrix operations, works with both
 * float and double (functionality only added when needed)
 */
class CMatrix {
  public:
    //////////////////////////////////////////////////////////////////////////////
    ...
    */
    template <typename T>
    static inline void copy(T **pptDestMat, const T *const *const pptSrcMat,
    ...
    \param iNumRows number of rows
    */
    template <typename T> static inline void free(T **&pptMat, int iNumRows) {
        if (!pptMat)
            return;
        assert(iNumRows > 0);
    ...
    */
    template <typename T>
    static inline void setCol(T **pptDestMat, const T *ptSrcVec, int iColIdx,
                              int iNumRows) {
        assert(pptDestMat);
        assert(ptSrcVec);
        assert(iColIdx >= 0);
    ...
    \param pptDest resulting matrix of dimension iNumRows x iNumCols (to be
    written, user allocated)
    ...
    */
    template <typename T>
    static inline void setRow(T **pptDestMat, const T *ptSrcVec, int iRowIdx,
                              int iNumCols) {
        assert(pptDestMat);
        assert(ptSrcVec);
        assert(iRowIdx >= 0);
    ...
    */
    template <typename T>
    static inline void setValue(T **pptMat, T tValue, int iNumRows,
                                int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumCols > 0);
        assert(pptMat);
    ...
    */
    template <typename T>
    static inline void setZeroBelowThresh(T **pptMat, int iNumRows,
                                          int iNumCols, T tThresh) {
        assert(iNumRows > 0);
        assert(iNumCols > 0);
        assert(pptMat);
        assert(pptMat[0]);
    ...
    \param pptSrcDest resulting matrix of dimension iNumRows x iNumCols (to be
    written, user allocated)
    \param iNumRows number of rows in the matrix
    \param iNumCols number of columns in the matrix
    */
    ...
    \param ptDest resulting vector of dimension iNumRows (to be written, user
    allocated)
    \param pptMat input matrix of dimension iNumRows x iNumCols (to be written,
    user allocated)
    \param iColIdx index of column to retrieve
    \param iNumRows number of rows in the matrix
    */
    template <typename T>
    static inline void getCol(T *ptDest, const T *const *const pptMat,
                              int iColIdx, int iNumRows) {
        assert(iColIdx >= 0);
        assert(iNumRows > 0);
        assert(pptMat);
        assert(ptDest);
    ...
    \param ptDest resulting vector of dimension min(iNumSrcRows, iNumCols) (to
    be written, user allocated)
    ...
    */
    template <typename T>
    static inline void getDiag(T *ptDest, const T *const *const pptSrc,
                               int iNumSrcRows, int iNumSrcCols) {
    ...
    */
    template <typename T>
    static inline T getMax(const T *const *const pptMat, int iNumRows,
                           int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumCols > 0);
        assert(pptMat);
    ...
    */
    template <typename T>
    static inline T getNorm(const T *const *const pptMat, int iNumRows,
                            int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumCols > 0);
        assert(pptMat);
    ...
    */
    template <typename T>
    static inline T getSum(T **pptMat, int iNumRows, int iNumCols,
                           bool bAbs = false) {
        assert(iNumRows > 0);
        assert(iNumCols > 0);
        assert(pptMat);
    ...
    */
    template <typename T>
    static inline T getSumCol(T **pptMat, int iColIdx, int iNumRows,
                              bool bAbs = false) {
        assert(iNumRows > 0);
        assert(pptMat);
        assert(pptMat[0]);
    ...
    */
    template <typename T>
    static T calcKlDivergence(const T *const *const pptSrc1,
                              const T *const *const pptSrc2, int iNumRows,
                              int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumCols > 0);
        assert(pptSrc1);
        ...
        for (int i = 0; i < iNumRows; i++) {
            for (int j = 0; j < iNumCols; j++)
                tResult += pptSrc1[i][j] * (std::log((pptSrc1[i][j] + 1e-24F) /
                                                     (pptSrc2[i][j] + 1e-24F)) -
                                            1.F) +
                           pptSrc2[i][j];
        }
    ...
    */
    template <typename T>
    static inline void add_I(T **pptSrcDest, const T *const *const pptSrc,
                             int iNumRows, int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumRows > 0);
        assert(pptSrcDest);
    ...
    */
    ...
    \param iNumRows number of rows
    \param iNumCols number of columns
    \param bAddSmallConst a small floating point number is added before division
    if true
    */
    template <typename T>
    static inline void div_I(T **pptSrcDest, const T *const *const pptSrc,
                             int iNumRows, int iNumCols,
                             bool bAddSmallConst = false) {
        assert(iNumRows > 0);
        assert(iNumRows > 0);
        assert(pptSrcDest);
        assert(pptSrcDest[0]);
    ...
    */
    template <typename T>
    static inline void mul_I(T **pptSrcDest, const T *const *const pptSrc,
                             int iNumRows, int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumRows > 0);
        assert(pptSrcDest);
    ...
    */
    template <typename T>
    static inline void mulC_I(T **pptMat, T tScale, int iNumRows,
                              int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumCols > 0);
        assert(pptMat);
    ...
    */
    ...
    \param pptDestMat resulting matrix of dimension iNumMatRows X iNumMatCols
    (to be written, user allocated)
    ...
    */
    template <typename T>
    static inline void mulColvecRowvec(T **pptDestMat, const T *ptSrcColVec,
                                       const T *ptSrcRowVec, int iNumMatRows,
                                       int iNumMatCols) {
    ...
    \param ptDestColVec resulting (column) vector of length iNumMatRows (to be
    written, user allocated)
    ...
    */
    template <typename T>
    static inline void
    mulMatColvec(T *ptDestColVec, const T *const *const pptMat,
                 const T *ptSrcColVec, int iNumMatRows, int iNumMatCols) {
        assert(iNumMatRows > 0);
        assert(iNumMatCols > 0);
        assert(ptDestColVec);
        assert(ptSrcColVec);
        ...
        for (auto m = 0; m < iNumMatRows; m++)
            ptDestColVec[m] =
                CVector::mulScalar(pptMat[m], ptSrcColVec, iNumMatCols);
    }

    /*! multiplies a matrix with a matrix (MAT1 * MAT2)
    \param pptDest resulting matrix of dimension iNum1Rows x iNum2Cols (to be
    written, user allocated)
    ...
    */
    template <typename T>
    static inline void mulMatMat(T **pptDest, const T *const *const pptSrc1,
                                 const T *const *const pptSrc2, int iNum1Rows,
                                 int iNum1Cols, int iNum2Rows, int iNum2Cols) {
        assert(iNum1Rows > 0);
        assert(iNum1Cols > 0);
        assert(iNum2Rows > 0);
        assert(iNum2Cols > 0);
    ...
    \param ptDestRowVec resulting (row) vector of length iNumMatCols (to be
    written, user allocated)
    ...
    */
    template <typename T>
    static inline void mulRowvecMat(T *ptDestRowVec, const T *ptSrcRowVec,
                                    const T *const *const pptMat,
                                    int iNumMatRows, int iNumMatCols) {
        assert(iNumMatRows > 0);
        assert(iNumMatCols > 0);
        assert(ptDestRowVec);
        assert(ptSrcRowVec);
    ...
    */
    template <typename T>
    static inline void sub_I(T **pptSrcDest, const T *const *const pptSrc,
                             int iNumRows, int iNumCols) {
        assert(iNumRows > 0);
        assert(iNumRows > 0);
        assert(pptSrcDest);
    ...
    static T det(const T *const *const pptMat, int iNumRows, int iNumCols) {
        ...
        if (iNumRows == 2)
            return ((pptMat[0][0] * pptMat[1][1]) -
                    (pptMat[0][1] * pptMat[1][0]));
            ...
            if (pptTmp[i][i] == 0) {
                for (j = i + 1; j < iNumCols; j++) {
                    if (pptTmp[i][j] != 0) {
                        ...
                        dDet *= pptTmp[i][i];
                        CVector::mulC_I(pptTmp[i], 1.F / pptTmp[i][i],
                                        iNumCols);
                        break;
                    }
                }
    ...
    */
    ...
    static inline void inv_I(T **pptSrcDest, int iNumRows, int iNumCols) {
        ...
        for (i = 1; i < iNumRows; i++) {
            ...
            while (j < i) {
                CVector::subW_I(ppfEye[i], ppfEye[j], pptSrcDest[i][j],
                                iNumCols);
                CVector::subW_I(pptSrcDest[i], pptSrcDest[j], pptSrcDest[i][j],
                                iNumCols);
                j++;
            }
            ...
            if (pptSrcDest[i][i] != 0) {
                ...
                CVector::mulC_I(pptSrcDest[i], 1.F / pptSrcDest[i][i],
                                iNumCols);
            }
        ...
        for (i = iNumRows - 1; i > 0; i--) {
            for (j = i - 1; j >= 0; j--) {
                CVector::subW_I(ppfEye[j], ppfEye[i], pptSrcDest[j][i],
                                iNumCols);
                CVector::subW_I(pptSrcDest[j], pptSrcDest[i], pptSrcDest[j][i],
                                iNumCols);
            }
        }
    ...
    */
    template <typename T>
    static inline void rearrangeRows(T **pptSrcDest, int *piRowIndices,
                                     int iNumRows) {
        assert(iNumRows > 0);
        assert(piRowIndices);
        assert(pptSrcDest);
    ...
    */
    template <typename T>
    static inline void swapRowCol(T **pptSrcDest, int iRowIdx, int iColIdx,
                                  int iNumCols) {
        assert(iRowIdx > 0);
        assert(iColIdx > 0);
        assert(iNumCols > 0);
    ...
    \param pptDest resulting matrix of dimension iNumSrcCols x iNumSrcRows (to
    be written, user allocated)
    \param pptSrc input matrix of dimension iNumSrcRows x iNumCols
    \param iNumSrcRows number of rows in the matrix
    \param iNumSrcCols number of columns in the matrix
    */
    template <typename T>
    static inline void transpose(T **pptDest, const T *const *const pptSrc,
                                 int iNumSrcRows, int iNumSrcCols) {
        assert(iNumSrcRows > 0);
        assert(iNumSrcCols > 0);
        assert(pptSrc);
