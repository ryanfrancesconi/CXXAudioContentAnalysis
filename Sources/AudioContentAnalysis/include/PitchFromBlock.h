...
/*! \brief class for computation of f0 from a block of data (e.g., time or
 * magnitude spectrum)
 */
class CPitchFromBlockIf {
  public:
    ...
    \return Error_t
    */
    static Error_t create(CPitchFromBlockIf *&pCInstance,
                          CPitchIf::PitchExtractors_t ePitchIdx,
                          int iDataLength, float fSampleRate = 1.F);
  ...
  protected:
    CPitchFromBlockIf() {};
    CPitchFromBlockIf(CPitchIf::PitchExtractors_t ePitchIdx, int iDataLength,
                      float fSampleRate);
    ;
    ...
    CPitchIf::PitchExtractors_t m_ePitchIdx =
        CPitchIf::kNumPitchExtractors; //!< index of Pitch to extract
