...
/*! \brief class for computation of a Novelty from a block of data (e.g., time
...
class CNoveltyFromBlockIf {
  public:
    ...
    \return Error_t
    */
    static Error_t create(CNoveltyFromBlockIf *&pCInstance,
                          CNoveltyIf::Novelty_t eNoveltyIdx, int iDataLength,
    ...
    */
    ...
    \return Novelty_t
    */
    CNoveltyIf::Novelty_t getNoveltyIdx() const { return m_eNoveltyIdx; }

    /*! performs the NoveltyFromBlock computation
    \param pfNovelty Novelty result (user-allocated, to be written, length from
    CNoveltyFromBlockIf::getNoveltyDimension)
    ...
    */
    ...
    ////////////////////////////////////////////////////////////////////////////
    // static functions for some Noveltys where it makes sense (use at your own
    // risk)
    ...
    */
    static float compNoveltyFlux(const float *pfMagSpec,
                                 const float *pfPrevSpec, int iDataLength,
                                 float fSampleRate = 1.F);
    /*! extracts novelty from a pair of neighboring magnitude spectra
    \param pfMagSpec magnitude spectrum of length iDataLength
    \param pfPrevSpec preceding magnitude spectrum of length iDataLength
    ...
    \return float novelty
    */
    static float compNoveltyHainsworth(const float *pfMagSpec,
                                       const float *pfPrevSpec, int iDataLength,
                                       float fSampleRate = 1.F);
    /*! extracts novelty from a pair of neighboring magnitude spectra
    \param pfMagSpec magnitude spectrum of length iDataLength
  ...
  protected:
    CNoveltyFromBlockIf() {};
    CNoveltyFromBlockIf(CNoveltyIf::Novelty_t eNoveltyIdx, int iDataLength,
                        float fSampleRate);
    ...
    CNoveltyIf::Novelty_t m_eNoveltyIdx =
        CNoveltyIf::kNumNoveltyFunctions; //!< index of Novelty to extract
    ...
    const std::map<
        ...
        std::function<float(const float *, const float *, int, float)>>
        m_DispatchMap{{CNoveltyIf::kNoveltyFlux, &compNoveltyFlux},
