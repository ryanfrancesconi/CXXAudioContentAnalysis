...
class CResample {
  public:
    /*! simple linear interpolation, static function can be used without
    instantiation
    \param pfOut buffer with interpolated output samples (user allocated of
    length iNumOutSamples)
    \param pfNewIdx indication of location of outputs on the input index scale,
    e.g., if the output sample rate is 3x the input sample rate, this vector
    will start with 0, 0.33, 0.66, 1, 1.33, ...
    \param pfIn original samples
    \param iNumOutSamples length of pfOut and pfNewIdx
    \param iNumInSamples length of pfIn
    \return Error_t
    */
    static Error_t interp1d(float *pfOut, const float *pfNewIdx,
                            const float *pfIn, long long iNumOutSamples,
                            long long iNumInSamples) {
        if (!pfNewIdx || !pfOut || !pfIn || iNumInSamples <= 0 ||
            iNumOutSamples <= 0)
            return Error_t::kFunctionInvalidArgsError;

        long long istart = 0, iEnd = iNumOutSamples - 1;
    ...
    explicit CResample(float fInSampleRate, float fOutSampleRate)
        : m_fInSampleRate(fInSampleRate), m_fOutSampleRate(fOutSampleRate) {
    ...
    virtual ~CResample() { delete m_pCFilter; }
    ...
    */
    long long getOutputLength(long long iNumInSamples) const {
        return CUtil::float2int<long long>(iNumInSamples / m_fInSampleRate *
                                           m_fOutSampleRate);
    }

    /*! does the sample rate conversion (filtered linear interpolation)
    \param pfOut output buffer (to be written, user allocated, length see
    CResample::getOutputLength)
    ...
    */
    Error_t process(float *pfOut, const float *pfIn, long long iNumInSamples) {
        ...
        const int iOrder = 4;
        float fOmegaCutoff = (m_fOutSampleRate > m_fInSampleRate)
                                 ? m_fInSampleRate / m_fOutSampleRate
                                 : m_fOutSampleRate / m_fInSampleRate;
        ...
        CButterLp::calcCoeffs(aafCoeffs[0], aafCoeffs[1], iOrder,
                              .9F * fOmegaCutoff);
        ...
        } else // downsample
        {
            ...
            // interpolate
            interp1d(pfOut, pfOutIdx, pfFiltered, iNumOutSamples,
                     iNumInSamples);
