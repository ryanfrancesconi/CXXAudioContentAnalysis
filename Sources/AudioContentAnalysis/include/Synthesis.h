...
class CSynthesis {
  public:
    ...
    */
    template <class T>
    static Error_t genSine(T *pfOutBuff, T fFreqInHz, T fSampleRateInHz,
                           long long iLen, T fAmplitude = 1.F,
                           T fStartPhaseInRad = 0.) {
        if (!pfOutBuff)
        ...
        for (int i = 0; i < iLen; i++) {
            pfOutBuff[i] =
                fAmplitude *
                static_cast<T>(sin(2 * M_PI * fFreqInHz * i / fSampleRateInHz +
                                   fStartPhaseInRad));
        }
    ...
    */
    template <class T>
    static Error_t genRect(T *pfOutBuff, T fFreqInHz, T fSampleRateInHz,
                           long long iLen, T fAmplitude = 1.) {
        if (!pfOutBuff)
            return Error_t::kFunctionInvalidArgsError;
   ...
   */
    template <class T>
    static Error_t genSaw(T *pfOutBuff, T fFreqInHz, T fSampleRateInHz,
                          long long iLen, T fAmplitude = 1.) {
        if (!pfOutBuff)
            return Error_t::kFunctionInvalidArgsError;
        ...
        for (int i = 1; i < iLen; i++) {
            pfOutBuff[i] =
                fmodf(pfOutBuff[i - 1] + fIncr + fAmplitude, 2 * fAmplitude) -
                fAmplitude;
        }
    ...
    */
    template <class T>
    static Error_t genNoise(T *pfOutBuff, long long iLen, T fAmplitude = 1.,
                            bool bOnlyPositive = true) {
        if (!pfOutBuff)
            return Error_t::kFunctionInvalidArgsError;
        ...
        if (bOnlyPositive) {
            for (int i = 0; i < iLen; i++)
                pfOutBuff[i] =
                    rand() * fAmplitude / static_cast<float>(RAND_MAX);
        } else {
            for (int i = 0; i < iLen; i++)
                pfOutBuff[i] =
                    rand() * 2 * fAmplitude / static_cast<float>(RAND_MAX) - 1;
        }
        return Error_t::kNoError;
    }
};
