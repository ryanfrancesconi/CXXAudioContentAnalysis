...
class CSeqFeatureSel {
  public:
    ...
    */
    ...
    \param ppfFeatures feature data for 'training' (dimensions iNumFeatures X
    iNumObs)
    \param piClassIndices ground truth class index for each observation
    \return Error_t
    */
    Error_t process(const float *const *const ppfFeatures,
                    const int *piClassIndices);

    /*! writes results after processing
    \param piFeatureIndices sorted feature indices, best first (dimensions
    iNumFeatures)
    \param pfAccuracyPerStep best accuracy for each combination (1 feature, 2
    features, 3 features...)
    \return Error_t
    */
  ...
  private:
    ...
    */
    ...
    float *m_pfAccuracy =
        0; //!< resulting best accuracy over number of features
    ...
    bool m_bWasProcessed =
        false; //!< indicates if process function had been successfully executed
};
