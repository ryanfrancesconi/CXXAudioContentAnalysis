...
class CViterbi {
  public:
    ...
    \param ppfPTransition transition probability matrix (dimension iNumStates X
    iNumStates)
    ...
    \return Error_t
    */
    Error_t init(const float *const *const ppfPTransition,
                 const float *pfPStart, int iNumStates, int iNumObs);
    ...
    */
    ...
    \param bUseLogLikelihood (use logarithmic likelihood - needed at least for
    longer sequences)
    \return Error_t
    */
    Error_t compViterbi(const float *const *const ppfPEmission,
                        bool bUseLogLikelihood = true);
    ...
    */
    ...
    \param pistateSequence pointer to memory the result is written to (user
    allocated, length iNumObs)
    \return Error_t
    */
