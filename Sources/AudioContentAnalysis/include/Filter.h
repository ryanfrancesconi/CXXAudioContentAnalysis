...
template <class T> class CFilter {
  public:
    ...
    */
    Error_t init(const T *pfB, const T *pfA, int iNumCoeffs) {
        ...
        m_pCFilterBuff->setWriteIdx(static_cast<int>(iNumCoeffs - 1));
    ...
    */
    Error_t reset(bool bFreeMem = true) {
        ...
        if (bFreeMem) {
            ...
            CVector::setZero(m_ptProcBuff,
                             m_iNumFilterCoeffs - static_cast<long long>(1));
        }

        return Error_t::kNoError;
    }

    /*! filters the signal (transposed direct form II implementation), can be
    ...
    Error_t process(T *pfOut, const T *pfIn, long long iNumSamples) {
        ...
        for (auto i = 0; i < iNumSamples; i++) {
            ...
            for (auto j = 1; j < m_iNumFilterCoeffs - 1; j++)
                m_ptProcBuff[j - 1] = m_ptProcBuff[j] -
                                      m_aptCoeff[kIir][j] * fTmp +
                                      m_aptCoeff[kFir][j] * pfIn[i];

            m_ptProcBuff[m_iNumFilterCoeffs - 2] =
                -m_aptCoeff[kIir][m_iNumFilterCoeffs - 1] * fTmp +
                m_aptCoeff[kFir][m_iNumFilterCoeffs - 1] * pfIn[i];
    ...
    /*! filters the signal (direct form II implementation, here as "informative"
     implementation)
     \param pfOut output signal (to be written, allocated by user)
     \param pfIn input signal
     \param iNumSamples length of both pfOut and pfIn
    ...
    Error_t processDFII(T *pfOut, const T *pfIn, long long iNumSamples) {
        ...
        for (auto i = 0; i < iNumSamples; i++) {
            ...
            for (auto j = 1; j < m_iNumFilterCoeffs; j++)
                fTmp -= m_aptCoeff[kIir][j] *
                        m_ptProcBuff[m_iNumFilterCoeffs - 1 - j];
    ...
    /*! zero phase filtering (can only be called with a complete signal, not
     block by block)
     \param pfOut output signal (to be written, allocated by user)
     \param pfIn input signal
     \param iNumSamples length of both pfOut and pfIn
    ...
    Error_t filtfilt(T *pfOut, const T *pfIn, long long iNumSamples) {
        ...
        for (auto i = 0; i < iPadLength; i++)
            pfPadding[i] =
                2 * pfIn[iNumSamples - 1] - pfIn[iNumSamples - 2 - i];
  ...
  private:
    ...
    // see sadovsky, bartusek, optimisation of the transient response of a
    // digital filter, radioengineering (9) no 2, 2000
    void setInitState_(float fWeight) {
        ...
        for (auto i = 1; i < m_iNumFilterCoeffs; i++)
            pfB[i - 1] =
                m_aptCoeff[kFir][i] - m_aptCoeff[kFir][0] * m_aptCoeff[kIir][i];
    ...
    CRingBuffer<T> *m_pCFilterBuff =
        0; //!< internal ring buffer for filter (only used for direct form II
           //!< implementation CFilter::processDFII)
...
/*! \brief class providing a generic filter implementation
 * implementation inspired by src from
 * https://exstrom.com/journal/sigproc/dsigproc.html
 */
class CButterLp {
  public:
    /*! computes buttworth lowpass coefficients
     \param pfB output signal (to be written, length iOrder+1, allocated by
     user)
    ...
    static Error_t calcCoeffs(T *pfB, T *pfA, int iOrder, T fCutOff) {
        const long long iTmpOrder = static_cast<long long>(
            iOrder); // silly compiler warnings get annoying
        float *pfTmpA = 0;
  ...
  private:
    template <typename T> static Error_t calcB(T *pfB, int iOrder, T fCutOff) {
        pfB[0] = 1;
    ...
    template <typename T> static Error_t calcA(T *pfA, int iOrder, T fCutOff) {
        ...
        for (auto j = 0; j < iOrder; j++) {
            ...
            T fNorm =
                static_cast<T>(1 + std::sin(M_PI * fCutOff) * std::sin(fArg));
            pfCoeff[2 * j] = static_cast<T>(-std::cos(M_PI * fCutOff) / fNorm);
            pfCoeff[2 * j + 1] = static_cast<T>(-std::sin(M_PI * fCutOff) *
                                                std::cos(fArg) / fNorm);
        }
    ...
    template <typename T> static T calcScale(int iOrder, T fCutOff) {
        T fPhase = static_cast<T>(M_PI / (2. * iOrder));
        T fScale = 1;

        for (auto j = 0; j < iOrder / 2; ++j)
            fScale *= static_cast<T>(1 + std::sin(M_PI * fCutOff) *
                                             std::sin((2 * j + 1) * fPhase));

        if (iOrder % 2)
            fScale *= static_cast<T>(std::sin(M_PI * fCutOff / 2) +
                                     std::cos(M_PI * fCutOff / 2));
        fScale = static_cast<T>(std::pow(std::sin(M_PI * fCutOff / 2), iOrder) /
                                fScale);
    ...
    static Error_t multBinomial(T *pfOut, const T *pfIn, int iOrder) {
        for (auto i = 0; i < iOrder; i++) {
            for (auto j = i; j > 0; j--) {
                pfOut[2 * j] += pfIn[2 * i] * pfOut[2 * (j - 1)] -
                                pfIn[2 * i + 1] * pfOut[2 * (j - 1) + 1];
                pfOut[2 * j + 1] += pfIn[2 * i] * pfOut[2 * (j - 1) + 1] +
                                    pfIn[2 * i + 1] * pfOut[2 * (j - 1)];
            }
            pfOut[0] += pfIn[2 * i];
            pfOut[1] += pfIn[2 * i + 1];
        }
