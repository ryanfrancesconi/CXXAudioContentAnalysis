...
class CDtw {
  public:
    ...
    */
    ...
    \param ppiPathResult pointer to memory the result is written to (dimensions
    [2][length_of_path])
    \return Error_t
    */
  ...
  private:
    ...
    \return Directions_t
    */
    static inline Directions_t findMin_(float fHorizCost, float fVertCost,
                                        float fDiagCost, float &fResultCost) {
    ...
    float *m_apfCost[kNumVectors] = {
        0,
        0}; //!< only allocate two rows instead of a whole matrix for the cost
    ...
    int m_aiMatrixDims[kNumMatrixDimensions] = {0,
                                                0}; //!< size of distance matrix

    static const int
        aiDecrement[kNumDirections]
                   [kNumMatrixDimensions]; //!< index changes for directions
