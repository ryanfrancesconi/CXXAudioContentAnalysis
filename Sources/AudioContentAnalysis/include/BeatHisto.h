...
/*! \brief class for computation of a BeatHistogram from either a file or a
 * vector
 */
class CBeatHistoIf {
  public:
    ...
    \return Error_t
    */
    static Error_t create(CBeatHistoIf *&pCInstance,
                          const std::string &strAudioFilePath,
                          int iBlockLength = 1024, int iHopLength = 8);
    ...
    \return Error_t
    */
    static Error_t create(CBeatHistoIf *&pCInstance, const float *pfAudio,
    ...
    */
    ...
    \param eBeatHistoComp (computation method used in
    CBeatHistoIf::compBeatHisto)
    \return Error_t
    */
    Error_t getNumBins(int &iNumBeatHistoBins,
    ...
    */
    ...
    \return Error_t
    */
    Error_t getBeatHistoAxisTicks(float *pfAxisTicks,
                                  BeatHisto_t eBeatHistoComp) const;

    /*! performs the BeatHisto computation for 1 dimensional BeatHistos and
    writes the result
    \param pfBeatHisto (user-allocated, to be written, dimensions from
    CBeatHistoIf::getNumBins)
    \param eBeatHistoComp computation method
    \return Error_t
    */
    Error_t compBeatHisto(float *pfBeatHisto,
                          BeatHisto_t eBeatHistoComp = kBeatHistoFft);

  protected:
    explicit CBeatHistoIf(int iBlockLength, int iHopLength, float fSampleRate)
        : m_iBlockLength(iBlockLength), m_iHopLength(iHopLength),
    ...
    Error_t init_(const float *pfAudio, long long iNumSamples,
                  float fSampleRate); //!< init configuration

    Error_t reset_(); //!< reset configuration

    void compHistoRange_(int &istartIdx, int &istopIdx,
                         BeatHisto_t eBeatHistoComp) const;
