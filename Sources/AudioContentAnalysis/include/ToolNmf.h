...
/*! \brief class holding the result of Non-Negative Matrix Factorization
 * executed with ::CNmf
 */
class CNmfResult {
    friend class CNmf;
  ...
  public:
    ...
    */
    ...
    /*! returns the number of rows in the matrix specified in eMatIdx
    \param ppfDest matrix to copy to (user allocated, dimensions see
    CNmfResult::getMatDims
  ...
  private:
    ...
    float **m_appfMatrices[kNumMatrices] = {
        0}; //!< arry holding all matrix pointers

    int m_aaiMatrixDims[kNumMatrices][2] = {
        {0}}; //!< array holding the various matrix dimensions

    bool m_bIsInitialized =
        false; //!< flag to indicate whether instance is initialized
};
...
class CNmf {
  public:
    ...
    \param pCNmfResult class holding the result to be initialized (user
    allocated)
    ...
    \return Error_t
    */
    Error_t init(CNmfResult *pCNmfResult, int iRank, int iNumRows, int iNumCols,
                 int iMaxIter = 300, float fSparsity = 0);
