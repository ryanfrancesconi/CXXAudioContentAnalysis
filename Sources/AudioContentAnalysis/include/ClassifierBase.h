...
class CClassifierBase {
  public:
    ...
    static const int
        kIllegalClassLabel; //!< indicates something went wrong (this number
                            //!< should not be used as a class label)
    ...
    */
    ...
    \param ppfTrainFeatures feature data for 'training' (dimensions iNumFeatures
    X iNumObs)
    \param piTrainClassIndices ground truth class index for each observation
    \param eNorm specification of what normalization should be applied to the
    feature data
    \return Error_t
    */
    virtual Error_t train(const float *const *const ppfTrainFeatures,
                          const int *piTrainClassIndices,
                          Normalization_t eNorm = kNoNormalization) = 0;
    ...
    */
    ...
    \return int class label of most likely class (returns
    CClassifierBase::kIllegalClassLabel in case of error)
    */
  ...
  protected:
    ...
    */
    virtual void compNormConstants(const float *const *const ppfTrainFeatures,
        ...
        if (eNorm == kZscoreNormalization) {
            for (auto f = 0; f < iNumFeatures; f++) {
                ...
                m_pfNormScale[f] = CVector::getStd(ppfTrainFeatures[f], iNumObs,
                                                   m_pfNormSub[f]);
            ...
            for (auto f = 0; f < iNumFeatures; f++) {
                ...
                m_pfNormScale[f] =
                    CVector::getMax(ppfTrainFeatures[f], iNumObs) -
                    m_pfNormSub[f];
                if (m_pfNormScale[f] != 0)
                    m_pfNormScale[f] = 1.F / m_pfNormScale[f];
            }
    ...
    /*! normalizes a new query vector according to previously extracted
    constants
    \param pfQuery vector of length iNumFeatures to classify
    \param iNumFeatures number of features (rows in the input matrix)
    */
    ...
    float *m_pfNormScale =
        0; //!< scaling constant for normalization (length m_iNumFeatures)
    float *m_pfNormSub =
        0; //!< offset for normalization (length m_iNumFeatures)
};
