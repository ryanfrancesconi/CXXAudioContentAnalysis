...
class CUtil {
  public:
    ...
    \return T
    */
    template <typename T> static T float2int(float fIn) {
        if (fIn >= 0.F)
    ...
    \return T
    */
    template <typename T> static T double2int(double fIn) {
        if (fIn >= 0)
            return static_cast<T>(fIn + .5);
        else
            return static_cast<T>(fIn - .5);
    ...
    \return bool
    */
    static bool isPowOf2(int n) { return !(n & (n - 1)); }
    ...
    */
    template <typename T> static void swap(T &tValue1, T &tValue2) {
        T tTmp = tValue1;
