...
class CAudioFileIf {
  public:
    ...
    enum FileFormat_t {
        ...
        kFileFormatWav,  //!< file is wav riff format (not available without
                         //!< sndlib)
    ...
    enum BitStream_t {
        ...
        kFileBitStreamFloat32, //!< a sample is four byte (float32)  (not
                               //!< available without sndlib)
    ...
    */
    ...
    \return Error_t
    */
    virtual Error_t openFile(std::string cAudioFileName, FileIoType_t eIoType,
                             FileSpec_t const *psFileSpec = 0) = 0;
    ...
    */
    ...
    \param iNumFrames: number of frames to read (per channel), is updated to the
    ...
    */
    ...
    \return Error_t
    */
    virtual Error_t writeData(const float *const *const ppfAudioData,
  ...
  protected:
    ...
    static const long long int
        m_kiDefBlockLength; //!< buffer length for read and write operations
  ...
  private:
    ...
    virtual long long int readDataIntern_(float **ppfAudioData,
                                          long long int iLength) = 0;
    virtual long long int
    writeDataIntern_(const float *const *const ppfAudioData,
                     long long int iLength) = 0;
    ...
    bool m_bWithClipping =
        false; //!< true if abs(values ) > 1 should be clipped
    bool m_bIsInitialized = false; //!< true if initialized
    int m_iNumBytesPerSample = 0;  //!< number of bytes per sample for the raw
                                   //!< pcm IO option without sndlib
};
