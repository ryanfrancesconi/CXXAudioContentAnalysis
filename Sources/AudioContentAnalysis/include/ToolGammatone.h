...
/*! \brief class for computation of a Gammatone filterbank output from either a
 * file or a vector
 */
class CGammaToneFbIf {
  public:
    ...
    \return Error_t
    */
    static Error_t create(CGammaToneFbIf *&pCInstance,
                          const std::string &strAudioFilePath,
                          int iNumBands = 20, float fStartInHz = 100);
    ...
    \return Error_t
    */
    static Error_t create(CGammaToneFbIf *&pCInstance, const float *pfAudio,
    ...
    \return Error_t
    */
    static Error_t create(CGammaToneFbIf *&pCInstance, float fSampleRate,
                          int iNumBands = 20, float fStartInHz = 100);
    ...
    */
    ...
    /*! performs the GammaToneFb computation for 1 dimensional GammaToneFbs and
    writes the result
    \param ppfOut (user-allocated, to be written, dimensions iNumBands x
    iNumSamples, see CGammaToneFbIf::getOutputDimensions)
    \param pfIn input data (for real-time block processing only)
    \param iNumSamples length of input data (for real-time block processing
    only)
    \return Error_t
    */
    Error_t process(float **ppfOut, const float *pfIn = 0,
  ...
  protected:
    ...
    float
    compMidFreqs_(float fFreqLow, float fFreqHigh,
                  int k) const; //!< compute center frequencies for all bands
