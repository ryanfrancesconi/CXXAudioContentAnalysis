...
class CKnn : public CClassifierBase {
  public:
    ...
    */
    ...
    \return Error_t
    */
    Error_t train(const float *const *const ppfTrainFeatures,
                  const int *piTrainClassIndices,
                  CClassifierBase::Normalization_t eNorm =
                      CClassifierBase::kNoNormalization) override;
  ...
  private:
    ...
    float *m_pfSortDist =
        0; //!< preallocated vector for distance of query to all data points
    float *m_pfQuery = 0; //!< preallocated vector for the normalized query
    int *m_piSortIdx =
        0; //!< preallocated vector for the resorted class indices

    float *m_pfHist =
        0; //!< preallocated vector for the histogram of nearest neighbors
    int *m_piHistLabel =
        0; //!< preallocated vector holding the histogram class labels
    int *m_piHistCount = 0; //!< number of entries per class (for weighted hist)

    bool m_bIsInitialized =
        false; //!< indicates if instance has been properly initialized
};
