...
class CLeaveOneOutCrossVal {
  public:
    ...
    */
    ...
    \return Error_t
    */
    float process(const float *const *const ppfTrainFeatures,
  ...
  private:
    CLeaveOneOutCrossVal(
        const CLeaveOneOutCrossVal &that); //!< disallow copy construction
    ...
    float **m_ppfTrain =
        0; //!< temporary train data (m_iNumObs X m_iNumFeatures)

    float *m_pfQuery =
        0; //!< preallocated vector for the normalized query (test vector)

    int *m_piClassLabels =
        0; //!< vector holding the temporary train set class labels
