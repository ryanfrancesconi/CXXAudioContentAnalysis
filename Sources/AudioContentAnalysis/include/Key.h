...
class CKey {
  public:
    ...
    \return Error_t
    */
    Error_t init(const std::string &strAudioFilePath, int iBlockLength = 4096,
                 int iHopLength = 2048);
    ...
    */
    Error_t init(const float *pfAudio, long long iNumSamples, float fSampleRate,
                 int iBlockLength = 4096, int iHopLength = 2048);
  ...
  protected:
    ...
    CFeatureIf *m_pCPitchChromaExtractor =
        0; //!< instance for extracting the pitch chroma
