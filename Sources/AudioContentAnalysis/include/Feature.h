...
/*! \brief class for computation of a instantaneous feature from either a file
 * or a vector, supports both one-dimensional and multi-dimensional features
 */
class CFeatureIf {
  public:
    ...
    \return Error_t
    */
    static Error_t create(CFeatureIf *&pCInstance, Feature_t eFeatureIdx,
    ...
    */
    static Error_t create(CFeatureIf *&pCInstance, Feature_t eFeatureIdx,
                          ...
                          float fSampleRate, int iBlockLength = 2048,
                          int iHopLength = 1024);
    ...
    */
    ...
    \param iNumRows (number of rows, to be written) equals number of feature
    ...
    */
    ...
    /*! performs the Feature computation for 1 dimensional features and writes
    the result
    \param pfFeature (user-allocated, to be written, dimensions from
    CFeatureIf::getFeatureDimensions)
    \return Error_t
    */
    Error_t compFeature1Dim(float *pfFeature);

    /*! performs the Feature computation for N dimensional features and writes
    the result
    \param ppfFeature (user-allocated, to be written, dimensions from
    CFeatureIf::getFeatureDimensions)
    \return Error_t
    */
  ...
  protected:
    ...
    bool
    isFeatureSpectral_(Feature_t eFeatureIdx); //!< bool differentiating between
                                               //!< time and spectral features
    void
    computeMagSpectrum_(); //!< compute magnitude spectrum for spectral features
    ...
    CFeatureFromBlockIf *m_pCFeature =
        0; //!< instance for feature computation with one block of data
