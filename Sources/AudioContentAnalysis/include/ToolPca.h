...
class CPca {
  public:
    ...
    */
    ...
    \param ppfRes matrix with principal components (user-allocated, to be
    written, dimensions equal input matrix)
    \param pfEigenValues eigenvalues of principle components (user-allocated, to
    be written, length equals number of features)
    \param ppfIn input matrix (dimensions iNumFeatures X iNumObs)
    \return Error_t
    */
    Error_t compPca(float **ppfRes, float *pfEigenValues,
                    const float *const *const ppfIn);

    /*! computes covariance matrix
    \param ppfCovOut resulting covariance matrix (user-allocated, to be written,
    dimension iNumRows X iNumRows)
    \param ppfIn input matrix (dimensions iNumRows X iNumCols), each row is a
    variable
    ...
    \return Error_t
    */
    static Error_t compCov(float **ppfCovOut, const float *const *const ppfIn,
    ...
    \return Error_t
    */
    static Error_t calcSVD(float **ppfU, float **ppfW, float **ppfV,
                           const float *const *const ppfMat, int iNumRows,
                           int iNumCols, int iMaxIterations = 100);
