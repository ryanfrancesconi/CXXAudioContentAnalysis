...
class CPreProc {
  public:
     ...
     */
    static void downmix(float *pfOut, const float *const *const ppfIn,
                        int iNumChannels, long long iNumSamples) {
        // sanity checks
        assert(pfOut);
        assert(ppfIn);
        assert(ppfIn[0]);
 ...
 * 1. normalize an audio file during blockwise reading with normalizeBlock
 * (requires instance with audio file)
 * 2. normalize an audio vector with blockwise reading with normalizeBlock
 * (requires instance with buffer)
 * 3. normalize an entire vector at once with normalizeSignal (static function
 * does not require instance)
 */
class CNormalizeAudio {
  public:
    explicit CNormalizeAudio(CAudioFileIf *pCAudioFile) : m_fScaleFactor(1.F) {
        assert(pCAudioFile);
        ...
        while (!pCAudioFile->isEof()) {
            ...
            CPreProc::downmix(ppfAudioData[0], ppfAudioData,
                              stFileSpec.iNumChannels, iNumSamples);
    ...
    /*! performs the normalization on a buffer after previous file parsing to
    get the maximum
