...
/*! \brief class for computation of a subfingerprint from a magnitude spectrum,
 * 256 subfingerprints comprise one fingerprint
 */
class CSubFingerprint {
  public:
    explicit CSubFingerprint(int iMagSpecLength = 1025,
                             float fSampleRate = 5000.F);
    ...
    */
    ...
    /*! performs the SubFingerprint computation
    \param pfSubFingerprint feature result (user-allocated, to be written,
    length from CSubFingerprint::getFeatureDimensions)
    \param pfMagSpec input data of length iDataLength
    \return Error_t
    */
    Error_t reset();
