...
class CInstFreq {
  public:
    explicit CInstFreq(int iFftLength, int iHopLength, float fSampleRate)
        : m_iHopLength(iHopLength), m_fSampleRate(fSampleRate) {
        ...
        CVector::alloc(m_pfSwapBuff,
                       2 * static_cast<long long>(m_iPhaseLength));
        ...
        for (auto k = 0; k < m_iPhaseLength; k++)
            m_pfOmega[k] =
                static_cast<float>(m_iHopLength * 2. * k * M_PI / iFftLength);
    }
    ...
    \param pfInstFreq output vector containing the estimated frequencies (to be
    written), length iFftLength/2+1
    ...
    */
    Error_t process(float *pfInstFreq, const CFft::complex_t *pfSpectrum) {
        ...
        CVector::sub_I(m_apfPhase[kPhasePrev], m_apfPhase[kPhaseCurr],
                       m_iPhaseLength);
        ...
        CVector::mulC_I(
            pfInstFreq,
            static_cast<float>(m_fSampleRate / (m_iHopLength * 2. * M_PI)),
            m_iPhaseLength);
  ...
  private:
    ...
    void princArg_(float *pfSrcDestPhase) {
        ...
        for (auto k = 0; k < m_iPhaseLength; k++)
            pfSrcDestPhase[k] = static_cast<float>(
                std::fmod(pfSrcDestPhase[k] + M_PI, -2 * M_PI) + M_PI);
    ...
    float *m_apfPhase[kNumPhases] = {
        0}; //!< pointers to buffer for storing the phase
    float *m_pfSwapBuff =
        0; //!< temporary process buffer and storage of previous phase spectrum
