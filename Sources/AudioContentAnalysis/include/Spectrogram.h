...
/*! \brief class for computation of a magnitude spectrogram from either a file
 * or a vector
 */
class CSpectrogramIf {
  public:
    ...
    \param pfWindow: window function of length iBlockLength (optional, default
    will be von-Hann if 0)
    \return Error_t
    */
    static Error_t create(CSpectrogramIf *&pCInstance,
                          const std::string &strAudioFilePath,
                          int iBlockLength = 2048, int iHopLength = 1024,
                          bool bNormalize = true, float *pfWindow = 0);
    ...
    \return Error_t
    */
    static Error_t create(CSpectrogramIf *&pCInstance, const float *pfAudio,
    ...
    */
    ...
    \param iNumRows (number of rows, to be written) equals number of frequency
    bins
    ...
    */
    ...
    /*! returns axis ticks
    \param pfAxisTicks (to be written) equals iNumRows if eAxisLabel ==
    kFrequencyInHz, otherwise iNumCols
    \param eAxisLabel indicator which axis
    \return Error_t
    */
    Error_t getSpectrogramAxisVectors(float *pfAxisTicks,
                                      enum AxisLabel_t eAxisLabel) const;

    /*! performs the Spectrogram computation
    \param ppfSpectrogram (user-allocated, to be written, dimensions from
    CSpectrogramIf::getSpectrogramDimensions)
    \return Error_t
    */
    Error_t compSpectrogram(float **ppfSpectrogram);

    /*! \brief structure holding configuration specifics for the mel spectrogram
     * computation
     */
    ...
    \return Error_t
    */
    Error_t getMelSpectrogramDimensions(
    ...
    */
    Error_t
    getMelSpectrogramAxisVectors(float *pfAxisTicks,
                                 enum AxisLabel_t eAxisLabel,
                                 const MelSpectrogramConfig_t *pMelSpecConfig);

    /*! performs the Spectrogram computation
    \param ppfMelSpectrogram (user-allocated, to be written, dimensions from
    CSpectrogramIf::getSpectrogramDimensions)
    \param pMelSpecConfig parametrization of the mel spectrogram
    \return Error_t
    */
    Error_t compMelSpectrogram(float **ppfMelSpectrogram,
                               const MelSpectrogramConfig_t *pMelSpecConfig);
  ...
  protected:
    ...
    CNormalizeAudio *m_pCNormalize =
        0; //!< instantiate if audio file normalization is wanted

    CBlockAudioIf *m_pCBlockAudio =
        0; //!< instantiate for blocking time domain signal
