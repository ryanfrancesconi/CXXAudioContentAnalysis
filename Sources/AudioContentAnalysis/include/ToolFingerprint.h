...
class CFingerprint {
  public:
    ...
    */
    ...
    \return Error_t
    */
    Error_t init(const float *pfAudio, long long iNumSamples,
                 float fSampleRate);
    ...
    */
    ...
    /*! returns time stamps
    \param pfAxisTicks (user- allocated, to be written) length from
    CFingerprint::getFingerprintLength
    \return Error_t
    */
    Error_t getTimeStamps(float *pfAxisTicks) const;

    /*! performs the Fingerprint extraction writes the result
    \param piFingerprint (user-allocated, to be written, length from
    CFingerprint::getFingerprintLength)
    \return Error_t
    */
  ...
  protected:
    ...
    CBlockAudioIf *m_pCBlockAudio =
        0; //!< instantiate for blocking the resampled time domain signal
    ...
    const float m_fProcSampleRate =
        5000.F;                   //!< sample rate for fingerprint extraction
