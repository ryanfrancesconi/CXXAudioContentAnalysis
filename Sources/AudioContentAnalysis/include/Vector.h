...
class CVector {
  public:
    ...
    */
    ...
    \param ptVec (empty pointer, to be allocated)
    */
    template <typename T> static inline void free(T *&ptVec) {
        delete[] ptVec;
        ptVec = 0;
    }
    ...
    */
    template <typename T>
    static inline void moveInMem(T *ptSrcDest, int iDestIdx, int isrcIdx,
                                 long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrcDest);

        if (iLength > 0)
            memmove(&ptSrcDest[iDestIdx], &ptSrcDest[isrcIdx],
                    sizeof(T) * iLength);
    }
    ...
    */
    template <typename T>
    static inline void setZeroBelowThresh(T *ptSrcDest, long long int iLength,
                                          T tThresh) {
        assert(iLength >= 0);
        assert(ptSrcDest);
    ...
    */
    template <typename T>
    static inline void add_I(T *ptSrcDest, const T *ptSrc,
                             long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrcDest);
        assert(ptSrc);
    ...
    */
    template <typename T>
    static inline void addW_I(T *ptSrcDest, const T *ptSrc, T fWeight,
                              long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrcDest);
        assert(ptSrc);
    ...
    */
    template <typename T>
    static inline void div_I(T *ptSrcDest, const T *ptSrc,
                             long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrcDest);
        assert(ptSrc);
    ...
    */
    template <typename T>
    static inline void mul_I(T *ptSrcDest, const T *ptSrc,
                             long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrcDest);
        assert(ptSrc);
    ...
    */
    template <typename T>
    static inline T mulScalar(const T *ptSrc1, const T *ptSrc2,
                              long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrc1);
        assert(ptSrc2);
    ...
    */
    template <typename T>
    static inline void sub_I(T *ptSrcDest, const T *ptSrc,
                             long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrcDest);
        assert(ptSrc);
    ...
    */
    template <typename T>
    static inline void subW_I(T *ptSrcDest, const T *ptSrc, T fWeight,
                              long long int iLength) {
    ...
    */
    template <typename T>
    static inline void sort_I(T *ptSrcDest, int *piIndices, int iLength,
                              bool bAscending = true) {
        // go bubble sort! ( should be replaced some time with something faster)

        bool bDone = false; // this flag will be used to check whether we have
        ...
        if (bAscending) {
            while (!bDone) {
                ...
                for (auto i = 0; i < iLength - 1; i++) {
                    if (ptSrcDest[i] >
                        ptSrcDest[i + 1]) // compare the current element with
                                          // the following one
                    {
                        // swap them
    ...
    */
    template <typename T>
    static inline T distCosine(const T *ptSrc1, const T *ptSrc2,
                               long long iLength) {
        assert(iLength >= 0);
        assert(ptSrc1);
        assert(ptSrc2);
        ...
        return 1.F - fDist / std::sqrt(mulScalar(ptSrc1, ptSrc1, iLength) *
                                       mulScalar(ptSrc2, ptSrc2, iLength));
    }
    ...
    */
    template <typename T>
    static inline T distEuclidean(const T *ptSrc1, const T *ptSrc2,
                                  long long iLength) {
        assert(iLength >= 0);
        assert(ptSrc1);
        assert(ptSrc2);
    ...
    */
    template <typename T>
    static inline T distManhattan(const T *ptSrc1, const T *ptSrc2,
                                  long long iLength) {
        assert(iLength >= 0);
        assert(ptSrc1);
        assert(ptSrc2);
    ...
    */
    template <typename T>
    static inline T getMax(const T *ptSrc, long long int iLength,
                           bool bAbs = false) {
        T fMax = 0;
        long long iMax = -1;
    ...
    */
    template <typename T>
    static inline T getMin(const T *ptSrc, long long int iLength,
                           bool bAbs = false) {
        T fMin = 0;
        long long iMin = -1;
    ...
    */
    ...
    \param ptSrc input buffer
    \param iLength number of elements in buffer
    \param fMean mean value if it has already been computed, otherwise it will
    be extracted in function
    \return T
    */
    template <typename T>
    static inline T getStd(const T *ptSrc, long long int iLength,
                           T fMean = std::numeric_limits<T>::max()) {
        assert(iLength >= 0);
    ...
    */
    template <typename T>
    static inline T getSum(const T *ptSrc, long long int iLength,
                           bool bAbs = false) {
        assert(iLength >= 0);
        assert(ptSrc);
    ...
    */
    template <typename T>
    static inline void findMax(const T *ptSrc, T &fMax, long long &iMax,
                               long long int iLength, bool bAbs = false) {
        assert(iLength >= 0);
        assert(ptSrc);
    ...
    */
    template <typename T>
    static inline void findMin(const T *ptSrc, T &fMin, long long &iMin,
                               long long int iLength, bool bAbs = false) {
        assert(iLength >= 0);
        assert(ptSrc);
     ...
     */
    template <typename T>
    static inline int findPeaks(bool *pbisLocalMax, const T *ptSrc,
                                long long int iLength,
                                T fThresh = -std::numeric_limits<T>::max()) {
        assert(iLength >= 0);
        assert(ptSrc);
        assert(pbisLocalMax);
        ...
        for (auto k = 1; k < iLength - 1; k++) {
            ...
            if (ptSrc[k] <= ptSrc[k - 1] || ptSrc[k] <= ptSrc[k + 1] ||
                ptSrc[k] <= fThresh)
                continue;
            else {
                pbisLocalMax[k] = true;
    ...
    */
    template <typename T>
    static inline bool isEqual(const T *ptSrc1, const T *ptSrc2,
                               long long int iLength) {
        assert(iLength >= 0);
        assert(ptSrc1);
        assert(ptSrc2);
