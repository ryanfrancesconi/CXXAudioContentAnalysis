...
class CSpectrogramFromFile : public CSpectrogramIf {
  public:
    CSpectrogramFromFile(std::string strAudioFilePath, int iBlockLength,
                         int iHopLength, bool bNormalize, float *pfWindow);
...
CSpectrogramFromFile::CSpectrogramFromFile(std::string strAudioFilePath,
                                           int iBlockLength, int iHopLength,
                                           bool bNormalize, float *pfWindow)
    : m_pCAudioFile(0) {
    this->reset_();
    ...
    CBlockAudioIf::create(m_pCBlockAudio, m_pCAudioFile, iBlockLength,
                          iHopLength);
...
class CSpectrogramFromVector : public CSpectrogramIf {
  public:
    CSpectrogramFromVector(const float *pfAudio, long long iAudioLength,
                           float fSampleRate, int iBlockLength, int iHopLength,
                           bool bNormalize, float *pfWindow);
    virtual ~CSpectrogramFromVector() {};
};

CSpectrogramFromVector::CSpectrogramFromVector(
    const float *pfAudio, long long iAudioLength, float fSampleRate,
    int iBlockLength, int iHopLength, bool bNormalize, float *pfWindow) {
    // set length variables
    m_iBlockLength = iBlockLength;
    m_iHopLength = iHopLength;
    ...
    CBlockAudioIf::create(m_pCBlockAudio, pfAudio, iAudioLength, iBlockLength,
                          iHopLength, m_fSampleRate);
...
/////////////////////////////////////////////////////////////////////////////////
// base class
CSpectrogramIf::CSpectrogramIf() { reset_(); }

inline CSpectrogramIf::~CSpectrogramIf() { reset_(); }

Error_t CSpectrogramIf::create(CSpectrogramIf *&pCInstance,
    ...
    pCInstance = new CSpectrogramFromFile(strAudioFilePath, iBlockLength,
                                          iHopLength, bNormalize, pfWindow);
...
Error_t CSpectrogramIf::create(CSpectrogramIf *&pCInstance,
                               ...
                               float fSampleRate, int iBlockLength,
                               int iHopLength, bool bNormalize,
                               float *pfWindow) {
    if (!pfAudio)
        return Error_t::kFunctionInvalidArgsError;
    if (iNumSamples <= 0)
    ...
    pCInstance = new CSpectrogramFromVector(pfAudio, iNumSamples, fSampleRate,
...
Error_t CSpectrogramIf::getSpectrogramDimensions(int &iNumRows,
                                                 int &iNumCols) const {
    if (!m_bIsInitialized) {
        iNumRows = 0;
        iNumCols = 0;
...
Error_t
CSpectrogramIf::getSpectrogramAxisVectors(float *pfAxisTicks,
                                          AxisLabel_t eAxisLabel) const {
    if (!m_bIsInitialized) {
        return Error_t::kFunctionIllegalCallError;
    }
...
void CSpectrogramIf::destroyMelFb_(
    const MelSpectrogramConfig_t *pMelSpecConfig) {
...
Error_t CSpectrogramIf::getMelSpectrogramDimensions(
    int &iNumRows, int &iNumCols,
    const MelSpectrogramConfig_t *pMelSpecConfig) const {
    if (!pMelSpecConfig)
        return Error_t::kFunctionInvalidArgsError;
...
Error_t CSpectrogramIf::getMelSpectrogramAxisVectors(
    float *pfAxisTicks, AxisLabel_t eAxisLabel,
    const MelSpectrogramConfig_t *pMelSpecConfig) {
    if (!m_bIsInitialized) {
        return Error_t::kFunctionIllegalCallError;
    }
...
Error_t CSpectrogramIf::compMelSpectrogram(
    float **ppfMelSpectrogram, const MelSpectrogramConfig_t *pMelSpecConfig) {
    if (!m_bIsInitialized)
        return Error_t::kFunctionIllegalCallError;
    if (!pMelSpecConfig)
        return Error_t::kFunctionInvalidArgsError;
    ...
    for (auto n = 0; n < iNumBlocks; n++) {
        ...
        for (auto k = 0; k < pMelSpecConfig->iNumMelBins; k++) {
            assert(m_ppfHMel[k]);
            ppfMelSpectrogram[k][n] =
                CVector::mulScalar(m_ppfHMel[k], m_pfSpectrum, iLength);
        }
        ...
        if (pMelSpecConfig->bIsLogarithmic) {
            ...
            for (auto k = 0; k < pMelSpecConfig->iNumMelBins; k++)
                ppfMelSpectrogram[k][n] =
                    20.F * std::log10(ppfMelSpectrogram[k][n] + 1e-12F);
        }
    }
...
Error_t
CSpectrogramIf::generateMelFb_(const MelSpectrogramConfig_t *pMelSpecConfig) {
    ...
    if (pMelSpecConfig->fMaxFreqInHz > m_fSampleRate / 2)
        return Error_t::kFunctionInvalidArgsError;
    if (pMelSpecConfig->iNumMelBins <= 0 ||
        pMelSpecConfig->iNumMelBins > iMagLength)
        return Error_t::kFunctionInvalidArgsError;

    // allocate filter matrix and frequency matrix
    CVector::alloc(m_pffcMel,
                   static_cast<long long>(pMelSpecConfig->iNumMelBins) +
                       2); // +2 for lower and upper bound
    ...
    m_pffcMel[pMelSpecConfig->iNumMelBins + 1] =
        CConversion::convertFreq2Mel(pMelSpecConfig->fMaxFreqInHz);
    float fMelInc =
        (m_pffcMel[pMelSpecConfig->iNumMelBins + 1] - m_pffcMel[0]) /
        (pMelSpecConfig->iNumMelBins + 1);
    ...
    float *pf_l = &m_pffcMel[0], *pf_c = &m_pffcMel[1], *pf_u = &m_pffcMel[2];

    for (auto m = 0; m < pMelSpecConfig->iNumMelBins; m++) {
        float fFilterMax = 2.F / (pf_u[m] - pf_l[m]); //!< normalization

        int iLowBin =
            1 + static_cast<int>(m_pCFft->freq2bin(pf_l[m], m_fSampleRate));
        int iCenterBin =
            1 + static_cast<int>(m_pCFft->freq2bin(pf_c[m], m_fSampleRate));
        int iUpBin =
            1 + static_cast<int>(m_pCFft->freq2bin(pf_u[m], m_fSampleRate));

        for (auto k = iLowBin; k < iCenterBin; k++)
            m_ppfHMel[m][k] = fFilterMax *
                              (CConversion::convertBin2Freq(
                                   static_cast<float>(k), (iMagLength - 1) * 2,
                                   m_fSampleRate) -
                               pf_l[m]) /
                              (pf_c[m] - pf_l[m]);
        for (auto k = iCenterBin; k < iUpBin; k++)
            m_ppfHMel[m][k] =
                fFilterMax *
                (pf_u[m] - CConversion::convertBin2Freq(static_cast<float>(k),
                                                        (iMagLength - 1) * 2,
                                                        m_fSampleRate)) /
                (pf_u[m] - pf_c[m]);
    }
