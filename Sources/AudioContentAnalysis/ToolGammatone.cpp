...
class CGammatone {
  public:
    ...
    */
    ...
    \param pfOut filter result (user-allocated, to be written, length
    iNumSamples)
  ...
  private:
    ...
    */
    ...
    enum { kNumCoeffs = 3, kNumFilters = 4 };
    ...
    CFilter<float> *m_apCFilter[kNumFilters] = {
        0}; //!< number crunching (multiple instances b/c cascaded)
...
Error_t CGammatone::process(float *pfOut, const float *pfIn,
                            long long iNumSamples) {
...
void CGammatone::calcFilterCoeffs_(int iOrder) {
    ...
    const double B = 1.019 * f2Pi *
                     std::pow(std::pow(m_fFreqCenter / fEarQ, iOrder) +
                                  std::pow(fBandwidth, iOrder),
                              1. / iOrder);
    ...
    m_aafCoeffB[0][1] = -(2. / m_fSampleRate * fCos / fExp +
                          fSqrtA / m_fSampleRate * fSin / fExp) /
                        2.;
    m_aafCoeffB[1][1] = -(2. / m_fSampleRate * fCos / fExp -
                          fSqrtA / m_fSampleRate * fSin / fExp) /
                        2.;
    m_aafCoeffB[2][1] = -(2. / m_fSampleRate * fCos / fExp +
                          fSqrtS / m_fSampleRate * fSin / fExp) /
                        2.;
    m_aafCoeffB[3][1] = -(2. / m_fSampleRate * fCos / fExp -
                          fSqrtS / m_fSampleRate * fSin / fExp) /
                        2.;
    ...
    // apply gain to first cascade
    {
        ...
        double fScale =
            2 * 2 * 2 * 2 /
            (m_fSampleRate * m_fSampleRate * m_fSampleRate * m_fSampleRate);
        std::complex<double> num1(-std::cos(2. * fArg), -std::sin(2 * fArg));
        std::complex<double> num2(std::cos(fArg) * std::exp(-B / m_fSampleRate),
                                  std::sin(fArg) *
                                      std::exp(-B / m_fSampleRate));
        std::complex<double> denom(-2. / std::exp(2. * B / m_fSampleRate) +
                                       2. / fExp +
                                       std::cos(2 * fArg) * (2. / fExp - 2.),
                                   std::sin(2 * fArg) * (2. / fExp - 2.));

        auto numerator = fScale * (num1 + num2 * (fCos - fSqrtS * fSin)) *
...
void CGammatone::calcFilterCoeffs_(int iOrder) {
    ...
    m_aafCoeffB[0][1] = -(2.F / m_fSampleRate * fCos / fExp +
                          fSqrtA / m_fSampleRate * fSin / fExp) /
                        2.F;
    m_aafCoeffB[1][1] = -(2.F / m_fSampleRate * fCos / fExp -
                          fSqrtA / m_fSampleRate * fSin / fExp) /
                        2.F;
    m_aafCoeffB[2][1] = -(2.F / m_fSampleRate * fCos / fExp +
                          fSqrtS / m_fSampleRate * fSin / fExp) /
                        2.F;
    m_aafCoeffB[3][1] = -(2.F / m_fSampleRate * fCos / fExp -
                          fSqrtS / m_fSampleRate * fSin / fExp) /
                        2.F;
    ...
    // apply gain to first cascade
    {
        ...
        std::complex<float> num2(std::cos(fArg) * std::exp(-B / m_fSampleRate),
                                 std::sin(fArg) * std::exp(-B / m_fSampleRate));
        std::complex<float> denom(-2.F / std::exp(2.F * B / m_fSampleRate) +
                                      2.F / fExp +
                                      std::cos(2 * fArg) * (2.F / fExp - 2.F),
                                  std::sin(2 * fArg) * (2.F / fExp - 2.F));
...
class CGammaToneFbFromFile : public CGammaToneFbIf {
  public:
    CGammaToneFbFromFile(const std::string &strAudioFilePath, int iNumBands,
                         float fStartInHz);
...
CGammaToneFbFromFile::CGammaToneFbFromFile(const std::string &strAudioFilePath,
                                           int iNumBands, float fStartInHz)
    : m_pCAudioFile(0)

{
    ...
    CBlockAudioIf::create(m_pCBlockAudio, m_pCAudioFile, m_iBlockLength,
                          m_iBlockLength);
...
/*! \brief class for computation of the filterbank outputs from a vector of
 * audio data
 */
class CGammaToneFbFromVector : public CGammaToneFbIf {
  public:
    CGammaToneFbFromVector(const float *pfAudio, long long iNumSamples,
                           float fSampleRate, int iNumBands, float fStartInHz);
    virtual ~CGammaToneFbFromVector() {};
};

CGammaToneFbFromVector::CGammaToneFbFromVector(const float *pfAudio,
                                               long long iNumSamples,
                                               float fSampleRate, int iNumBands,
                                               float fStartInHz) {
    m_iNumBands = iNumBands;
    m_fSampleRate = fSampleRate;
    m_fStartInHz = fStartInHz;

    CBlockAudioIf::create(m_pCBlockAudio, pfAudio, iNumSamples, m_iBlockLength,
                          m_iBlockLength, m_fSampleRate);
...
/*! \brief class for computation of the filterbank outputs in real time block by
 * block
 */
...
CGammaToneFbRealTime::CGammaToneFbRealTime(float fSampleRate, int iNumBands,
                                           float fStartInHz) {
    m_iNumBands = iNumBands;
    m_fSampleRate = fSampleRate;
    m_fStartInHz = fStartInHz;
...
/////////////////////////////////////////////////////////////////////////////////
// base class
CGammaToneFbIf::CGammaToneFbIf() { reset_(); }

inline CGammaToneFbIf::~CGammaToneFbIf() { reset_(); }

Error_t CGammaToneFbIf::create(CGammaToneFbIf *&pCInstance,
                               const std::string &strAudioFilePath,
                               int iNumBands, float fStartInHz) {
    ...
    pCInstance =
        new CGammaToneFbFromFile(strAudioFilePath, iNumBands, fStartInHz);

    return Error_t::kNoError;
}

Error_t CGammaToneFbIf::create(CGammaToneFbIf *&pCInstance,
                               const float *pfAudio, long long iNumSamples,
                               float fSampleRate, int iNumBands,
                               float fStartInHz) {
    if (!pfAudio)
        return Error_t::kFunctionInvalidArgsError;
    ...
    pCInstance = new CGammaToneFbFromVector(pfAudio, iNumSamples, fSampleRate,
                                            iNumBands, fStartInHz);

    return Error_t::kNoError;
}

Error_t CGammaToneFbIf::create(CGammaToneFbIf *&pCInstance, float fSampleRate,
                               int iNumBands, float fStartInHz) {
    if (fSampleRate <= 0)
        return Error_t::kFunctionInvalidArgsError;
    if (iNumBands <= 0)
...
Error_t CGammaToneFbIf::getOutputDimensions(long long &iNumRows,
                                            long long &iNumCols) const {
    if (!m_bIsInitialized) {
        iNumRows = 0;
        iNumCols = 0;
        return Error_t::kFunctionIllegalCallError;
...
Error_t CGammaToneFbIf::process(float **ppfOut, const float *pfIn /*= 0*/,
                                long long iNumSamples /*= 0*/) {
    if (!m_bIsInitialized)
        return Error_t::kFunctionIllegalCallError;
    if (!ppfOut)
        return Error_t::kFunctionInvalidArgsError;
    ...
    for (auto n = 0; n < iNumBlocks; n++) {
        ...
        for (auto i = 0; i < m_iNumBands; i++) {
            assert(ppfOut[i]);
            m_ppCGammatone[i]->process(&ppfOut[i][n * m_iBlockLength],
                                       m_pfProcBuff, iNumFrames);
        }
    }
...
Error_t CGammaToneFbIf::init_() {
    ...
    for (auto i = 0; i < m_iNumBands; i++) {
        ...
        m_ppCGammatone[i]->init(
            compMidFreqs_(m_fStartInHz, m_fSampleRate / 2, i), m_fSampleRate);
    }
...
inline float CGammaToneFbIf::compMidFreqs_(float fFreqLow, float fFreqHigh,
                                           int k) const {
    const float fEarQ = 9.26449F;
    const float fBandwidth = 24.7F;

    return -(fEarQ * fBandwidth) +
           std::exp((m_iNumBands - k) *
                    (-std::log(fFreqHigh + fEarQ * fBandwidth) +
                     std::log(fFreqLow + fEarQ * fBandwidth)) /
                    m_iNumBands) *
               (fFreqHigh + fEarQ * fBandwidth);
}
