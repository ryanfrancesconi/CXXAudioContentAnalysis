...
CSubFingerprint::CSubFingerprint(int iMagSpecLength, float fSampleRate)
    : m_iMagSpecLength(iMagSpecLength), m_fSampleRate(fSampleRate) {
    ...
    for (auto k = 0; k < kNumProcBuffs; k++)
        CVector::alloc(m_apfProcBuff[k],
                       m_iNumBands + static_cast<long long>(1));
...
uint32_t CSubFingerprint::compSubFingerprint(const float *pfMagSpec) {
    ...
    CVector::setZero(m_apfProcBuff[kCurr],
                     m_iNumBands + static_cast<long long>(1));

    for (auto p = 0; p < m_iNumBands + 1; p++) {
        // we could do this nicer with CVector::mulScalar if we allocated
        // memory; or, we could use the bin boundaries for start and end of
        // loop. or both.
        for (auto k = 0; k < m_iMagSpecLength; k++)
            m_apfProcBuff[kCurr][p] +=
                m_ppfH[p][k] * (pfMagSpec[k] * pfMagSpec[k]);
    }

    // compute diff in freq domain
    for (auto p = 0; p < m_iNumBands; p++) {
        m_apfProcBuff[kCurr][p] =
            m_apfProcBuff[kCurr][p + 1] - m_apfProcBuff[kCurr][p];
        m_apfProcBuff[kTmp][p] =
            m_apfProcBuff[kCurr][p] - m_apfProcBuff[kPrev][p];
    }
...
Error_t CSubFingerprint::reset() {
    ...
    for (auto k = 0; k < kNumProcBuffs; k++)
        CVector::setZero(m_apfProcBuff[k],
                         m_iNumBands + static_cast<long long>(1));
...
void CSubFingerprint::genBands_() {
    ...
    for (auto p = 0; p < m_iNumBands; p++) {
        float fHighBound =
            afFreqBounds[kLow] * std::exp((p + 1) * fFreqScale / iNumBands);
        // get indices from freqs
        const int aiBoundIdx[kNumBounds] = {
            static_cast<int>(CConversion::convertFreq2Bin(fLowBound, iFftLength,
                                                          m_fSampleRate)) +
                1,
            static_cast<int>(CConversion::convertFreq2Bin(
                fHighBound, iFftLength, m_fSampleRate))};

        CVector::setValue(&m_ppfH[p][aiBoundIdx[kLow]], 1.F,
                          aiBoundIdx[kHigh] + static_cast<long long>(1) -
                              aiBoundIdx[kLow]);
