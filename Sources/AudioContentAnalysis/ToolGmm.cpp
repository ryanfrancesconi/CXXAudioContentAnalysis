...
Error_t CGmm::init(CGmmResult *pCResult, int iK, int iNumFeatures, int iNumObs,
                   int iMaxIter) {
...
Error_t CGmm::compGmm(CGmmResult *pCResult,
                      const float *const *const ppfFeatures) {
...
void CGmm::initState_(const float *const *const ppfFeatures,
                      CGmmResult *pCCurrState) {
    // generate some noise
...
void CGmm::compProbabilities_(const float *const *const ppfFeatures,
                              CGmmResult *pCCurrState) {
    // compute gaussian per cluster per observation
    for (auto k = 0; k < m_iK; k++) {
        pCCurrState->getSigma(m_appfSigma[0], k);
        float fDet =
            CMatrix::det(m_appfSigma[0], m_iNumFeatures, m_iNumFeatures);
        if (fDet < 1e-30F) {
            CVector::setZero(m_ppfProb[k], m_iNumObs);
            continue;
        }

        float fNorm = static_cast<float>(
            1. / std::sqrt(std::pow(2. * M_PI, m_iNumFeatures) * fDet));
        ...
        for (auto n = 0; n < m_iNumObs; n++) {
            ...
            CMatrix::mulMatColvec(m_apfProc[1], m_appfSigma[0], m_apfProc[0],
                                  m_iNumFeatures, m_iNumFeatures);

            m_ppfProb[k][n] =
                fNorm *
                std::exp(-.5F * CVector::mulScalar(m_apfProc[0], m_apfProc[1],
                                                   m_iNumFeatures));
        }
...
void CGmm::updateState_(const float *const *const ppfFeatures,
                        CGmmResult *pCCurrState) {
    for (auto k = 0; k < m_iK; k++) {
        // update priors
        ...
        for (auto v = 0; v < m_iNumFeatures; v++)
            pCCurrState->setMu(
                k, v,
                CVector::mulScalar(ppfFeatures[v], m_ppfProb[k], m_iNumObs) /
                    fNorm);
        ...
        for (auto n = 0; n < m_iNumObs; n++) {
            ...
            CMatrix::mulColvecRowvec(m_appfSigma[1], m_apfProc[0], m_apfProc[0],
                                     m_iNumFeatures, m_iNumFeatures);
            CMatrix::mulC_I(m_appfSigma[1], m_ppfProb[k][n], m_iNumFeatures,
                            m_iNumFeatures);
            CMatrix::add_I(m_appfSigma[0], m_appfSigma[1], m_iNumFeatures,
                           m_iNumFeatures);
        }
        fNorm = CVector::getSum(m_ppfProb[k], m_iNumObs);
        if (fNorm < 1e-30F)
            fNorm = 1.F;
        CMatrix::mulC_I(m_appfSigma[0], 1.F / fNorm, m_iNumFeatures,
                        m_iNumFeatures);
        pCCurrState->setSigma(k, m_appfSigma[0]);
    }
}
...
CGmmResult &CGmmResult::operator=(const CGmmResult &that) {
    ...
    if (this->m_iK != that.m_iK ||
        this->m_iNumFeatures != that.m_iNumFeatures ||
        this->m_bIsInitialized != that.m_bIsInitialized)
        this->init(that.m_iK, that.m_iNumFeatures);

    CMatrix::copy(this->m_ppfMu, that.m_ppfMu, this->m_iK,
                  this->m_iNumFeatures);
    CVector::copy(this->m_pfPrior, that.m_pfPrior, this->m_iK);
    for (auto i = 0; i < kSigma; i++) {
        for (auto k = 0; k < this->m_iK; k++)
            CMatrix::copy(this->m_apppfSigma[i][k], that.m_apppfSigma[i][k],
                          this->m_iNumFeatures, this->m_iNumFeatures);
    }
...
float CGmmResult::getProb(const float *pfQuery) {
    float fProb = 0;
    for (auto k = 0; k < m_iK; k++) {
        float fDet = CMatrix::det(m_apppfSigma[kNormal][k], m_iNumFeatures,
                                  m_iNumFeatures);
        if (fDet < 1e-30F)
            continue;
        float fNorm = static_cast<float>(
        ...
        CMatrix::mulMatColvec(m_apfProc[1], m_apppfSigma[kInv][k], m_apfProc[0],
                              m_iNumFeatures, m_iNumFeatures);

        fProb += this->getPrior(k) * fNorm *
                 std::exp(-.5F * CVector::mulScalar(m_apfProc[0], m_apfProc[1],
                                                    m_iNumFeatures));
    }
...
CGmmResult::CGmmResult(const CGmmResult &that)
    : m_iK(that.m_iK), m_iNumFeatures(that.m_iNumFeatures),
    ...
    for (auto i = 0; i < kSigma; i++) {
        ...
        for (auto k = 0; k < m_iK; k++) {
            ...
            CMatrix::copy(m_apppfSigma[i][k], that.m_apppfSigma[i][k],
                          m_iNumFeatures, m_iNumFeatures);
        }
    }
}

int CGmmResult::getNumGaussians() const { return m_iK; }

int CGmmResult::getNumDimensions() const { return m_iNumFeatures; }
...
void CGmmResult::getSigma(float **ppfSigma, int iGaussianIdx) const {
    CMatrix::copy(ppfSigma, m_apppfSigma[kNormal][iGaussianIdx], m_iNumFeatures,
                  m_iNumFeatures);
    return;
}

bool CGmmResult::isInitialized() const { return m_bIsInitialized; }
...
Error_t CGmmResult::setMu(int iGaussianIdx, int iFeatureIdx,
                          float fParamValue) {
...
Error_t CGmmResult::setSigma(int iGaussianIdx, float **ppfSigma) {
    CMatrix::copy(m_apppfSigma[kNormal][iGaussianIdx], ppfSigma, m_iNumFeatures,
                  m_iNumFeatures);

    CMatrix::copy(m_apppfSigma[kInv][iGaussianIdx], ppfSigma, m_iNumFeatures,
                  m_iNumFeatures);
    CMatrix::inv_I(m_apppfSigma[kInv][iGaussianIdx], m_iNumFeatures,
                   m_iNumFeatures);
